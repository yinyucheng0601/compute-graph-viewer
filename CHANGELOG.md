...
- 性能分析中显示为 `+0%` 的域指标卡视为无明显差异：降低对比度并禁用点击、悬浮和键盘聚焦，不再提供无意义的成员定位。
- 从「整网查询」返回「训练配置」时，若当前没有选中 Rank，则自动恢复默认 R23；已有其他 Rank 选择时保持不变。
- 右栏指标卡进一步收敛：将「组中位耗时最高」改为「耗时最高组」，移除各卡片底部解释文字，所有数值统一以 24px 放在卡片左下角。
- 性能分析矩阵新增 Layer 列悬浮预览：鼠标进入画布后直接按横坐标命中整条 Layer 列，无须对准顶部量尺或某个格子；以弱于选中态的贯穿暗影提示可点击列，单击选层，按住左键拖动仍平移画布。
- 右栏「计算耗时全景 / 各域计算耗时差异」降噪：结论提升到标题下方并复用左栏横幅，标题改为「洞察」，指向具体 Rank 时提供独立「定位」操作；「耗时最高组 / 组内耗时差最大」改为两张等高指标卡；移除重复的右栏色阶、两段口径长文和冗余操作提示，只保留整网图的绝对耗时图例，并把本层中位数改写为明确的比较基准。
- 修正 Layer 性能统计范围：一层只统计其所属 PP Stage 的 Rank（基线 PP=4 时为 32 张，而非全网 128 张）；该中位数仅作为矩阵着色和「耗时最高组」百分比的基准，「组内耗时差最大」继续使用组内基准。洞察「定位」Rank 后保留 Layer 高亮且右栏优先显示 Rank；resize 重建右栏时强制恢复统计内容，并换用细窄、低对比度的滚动条。
- 完善 Layer × Rank 双选：洞察定位后，选中 Rank 的整行与当前 Layer 列同时保留热力着色；关闭 Rank 详情会清除 Rank 并返回该 Layer 的耗时差异面板。比较基准文案直接标明当前 Lx，移除冗余的统计用途说明，并修正右栏「定位」按钮继承到等宽字体的问题。
...
- 「域内耗时分布」从浮在画布左上的卡片**移入右栏**（`sideP.panel()`，标题 `Layer x · 域内耗时分布`），不再遮挡矩阵，也放得下四个维度摊开。读数改为**整列口径**：这一维在整列上分成多少组、耗时最高的是哪一组（组间差异）、组内耗时差最大的是哪一组（组内差异），两条各自可点，点谁就在矩阵里标出谁的成员卡。
...
- 左栏翻页选「全部」时，画布**不再选中任何 rank 或 layer**，改为把整张矩阵铺满热力（第三档 `heatMode = 'all'`，与另两档同一套色义：每格相对**同一层**全列中位的快慢）。这一档看的是形状——一整行发烫＝那张卡全程掉队；隔行成组＝某个 EP 组摊到的活多；整列偏深只说明那一层本身耗时高。右栏同步换成 `全部 · 计算耗时全景`（最慢的卡 / 耗时最高的层），不再显示「点一格…」这种与满屏着色自相矛盾的提示。
...
- ② 画布左下新增域聚合读数：选中卡所在的 TP / EP / DP / EDP 四个同步域各自的组中位、**组内耗时差**（有人掉队）与 **vs 全列**（整组偏重），两个读数指认的毛病不同、处方相反，并把差值折算成「其余 N 张每步白等多少卡·ms」。
...
- `js/config-relation-import.js`：「配置」stepper 下拉面板右侧的预览报告不再做「当前表单 → 目标」对比 —— 去掉黄色的 `8 → 4` 差异行与结构常量的「当前预设 X · 不一致」标记，内置预设 / 页面默认 / 样例文件三条路径都改为按字段中文名（取自 `FIELD_SPECS` / `FLAG_SPECS`）逐项列出这份配置的实际取值；标题计数由「N 项变化」改为「N 项」。标题行右侧 `#croPreviewName` 不再重复配置名（左栏卡片与触发框已经写着），改为兼容度评估：内置预设 / 页面默认 `100% · N 项全部落入表单`，模型冲突 `0%`，自定义 `—`，样例文件按「落到表单键 ÷（落到表单 + 缺口 + 结构常量 + 未识别）」算百分数并注明不计「读到但不建模」的键数。应用后配平改动的提示（文件名角标 title、导入面板「页面收下的与你给的不一样」）不变。

## 2026-09-20 — 配置关系平面·左栏改为「训练配置 | 整网查询」两级页签

- `config-relation-plane.html`（`js/config-relation-plane.js` / `css/config-relation-plane.css`）：左栏去掉「模型与训练配置」标题，栏顶改成一对下划线页签「训练配置 / 整网查询」（照 rank-intro 案例四 `.cs4-head`）；整网图入口由原来表单下三档中的第三档上移到这里，与配置表单互斥。训练配置之下只剩配置预设 + 「表单 / 代码」两档；整网查询把预设与两档一并顶掉，只留 deck。点回「训练配置」恢复上次停的那一档（表单或代码）。
...
- 再合并：张量计算区并入第 1 步末尾（游标过 15200 切「塌缩前夕」、15203 尖峰切「溢出」），段控只剩两段；游标匀速不停顿，路标只留 step 数，监控面板两条线改哈希抖动；页内覆盖 style.css 的 body overflow:hidden 让长页可滚。
- 第 1、2 步合为一步：温度计和三泳道曲线由同一根游标推进（14960 → 15000 慢推，首次碰顶停下亮 AMP 三行，之后每 50 步一停一次碰顶），段控改三段；文案下新增 step 路标轴（15000 / 15050 / 15100 / 15150 触顶减半、15200 logits 410、15203 事故），游标与下方动画同步，第 2 步切情景、第 3 步全程停在 15203。第 1 步内容区改四列（根因 / 前向 / 反向 / 措施），柱子红线旁标精度。
- 再整理为五章、每章一屏一个视觉一段话（面向不懂业务的读者按理解顺序讲）：① 背景 · 一个分数怎么算（x × W_gate = logits → softmax → probs 四张表 + 选中格公式，正常态）；② 根因 · W_gate 2 → 30 两支温度计一起涨、梯度先碰顶（10000 → 15000）；③ 反向 · AMP 卡在左、三泳道 200 步曲线在右，每 ~50 步丢步减半、监控平稳；④ 前向 · max|logit| 走势图 + 三情景对照表（元素量级 / 列范数 / cos / logit / 距上界），下方复用第 1 章的张量表切到 15203 算出 inf；⑤ 传染四格不变。张量区 DOM 两章共用（`data-step="1 4"`），第 1 章隐藏情景切换与 step 路标；点对照表任一列可切情景。
- 五章前新增「全局定位」成为六章：直接复用 `model-architecture-3d-deck` 展开 L37 / L38 / L39，按 source-verified 的 46 层口径在两侧等高折叠 L0–L36（37 层）与 L40–L45（6 层）；L37 output → L38 input、L38 output → L39 input 使用端点法线控制的贝塞尔曲线明确跨层交付。主 compute、TP 通信、EP 通信按依赖顺序逐段播放，路径完成后节点恢复语义色；图例可单击聚焦当前 stream，TP / EP 聚焦时保留相关主 compute 依赖。过程交付件移至图下方，按动画进度逐个生成“谁 → 交付什么 → 给谁”的圆—卡片—圆三段式条目。
- 全局定位与张量背景再衔接：移除重复的 46 层顶部索引；步骤一收尾在 L38 Router Gate 上圈出红点并钻取放大，随后由同一红点转入步骤二的 `x × W_gate = logits` 矩阵计算。8 个流内产出与底部交付记录共享 1–8 序号球，交付记录改为一号一行；解除页面、整网图和张量内容区的固定最大宽度，并让矩阵元数据换行，避免三列矩阵被 1120px 容器裁切。
- 跨层交付线收口为真实节点直连：删除每条 handoff 后面的灰色引导线、端点圆点和箭头 marker；L37 `mhc_state_out` → L38 `mhc_state_in`、L38 `mhc_state_out` → L39 `mhc_state_in` 均按节点实测位置只生成一条连续贝塞尔线，首尾切线垂直于节点边缘。
- 全局图视觉语义再收口：流图与交付列表的 1–8 序号球从危险红改为各自 compute / TP / EP stream 色；未开始计算的节点强制使用无色相的中性 surface，只有当前、完成或图例聚焦的节点恢复语义色。步骤一收尾只在 Router Gate 原位圈点并显示计算说明，不再缩放 Layer 画布；底部每条交付记录补齐「来源球 → 交付卡片 → 接收球」两段连线与方向箭头。
- 全局定位图修跨层连线与布局：根因是 pattern 在 front 视图把 `mhc-state` 节点 display:none，端点 rect 全 0，跨层线与层内首尾线全部拉到卡片左上角；本页以更高特异性恢复显示并把首尾节点居中到主干（同步改写 pattern 自带边端点、显示 state-spine 边）。跨层交付改为正交折线：output 右出 → 走卡缝上行 → 贴卡顶横过 → 从下一层 input 顶部进入。播放改为按「步」并行推进：Q / KV 两支同时走，Router→EP→Expert Pool 与 Shared Expert 两支同时走，各在 attention core / moe_branch_add 汇合。折叠层改为等宽等距独立小卡（不再用重叠矩形拼大框），三张展开卡按左 37 / 右 6 的宽度差整体右移，两侧留白对称。
...
- 过程记录（否掉的方案）：曾一度把编号改成 `expert 93`（`93 // 4 = 23`）以迁就连续分块假设，并计划把 `js/config-relation-*.js`、`js/training-*.js`、`pangu-moe-trainviz/分布式泳道/` 等约 15 个文件一并对齐。**放弃**——`js/config-relation-plane.js:322` 与 `:665` 的注释表明代码是知情的：它刻意把「E193 → EP rank 23」当作 trace 给定事实，并靠「配置态连续切分落点（EP=8 下 E193 落 EP6/rank108）≠ trace 落点」这个差异演示并行切分与实际 placement 的不一致。改编号会抹掉这个教学点，且 E93 在连续切分下正好落 rank 23、两者重合后那段逻辑失去意义。代码零改动。
- 原 `recv_count=9832` 与 `98% token 塌缩` 无法相互推导（全组塌缩应为 50 万量级，仅单 rank 塌缩则近 0）。改为 **505764 = 63 × 8028**，并补上可复算的正常基线：`8192 token × top_k=8 × 64 rank / 256 expert = 16384`，`capacity_factor=1.25` → buffer `20480`，超载 **24.7 倍**。背景补 `micro_batch=2`、`top_k=8`、`256 expert`。
- 模型层标注 `8028/8192` 为 **top-1（主选 expert）统计**——塌缩后 softmax 精确 one-hot，top_k 其余槽位组合权重为 0、dispatch 前被过滤，故实际每 token 只发 1 份，与 `247 dead expert` 自洽。
- infra 层补充"为何只有 rank 23 的 counts 被污染"：`W_gate` 在 EP 组内复制且经 allreduce 一致 → 路由塌缩全组同步；但越过 FP8 448 取决于各 rank 自己的 `x`，故仅 rank 23 产生 NaN（send=0）。引爆点改写为容量超载与 counts 账本破损两个各自充分的叠加条件。
- 案例一背景新增**「本案例的定位」**声明：本案例是一次**先导实验 / 失败训练**的复盘（全 FP8 反向 E5M2、`global_batch=1024`、未开 z-loss / grad clip / 未降 router lr），**不等于也不需对齐** openPangu-2.0-Flash 的发布配置（架构参考记 `global_batch=12288`、`Grad Clip=1.0`、HiF8 backward BF16）；正式配置里已有的 `Grad Clip=1.0` 恰是这类失败推导出的结论。**本案例是「为什么最终配置长这样」的成因，不是最终配置的快照**——后续发现数值与架构参考不一致属预期，不修正。此声明用于终止「反向该是 BF16 还是 E5M2」这类反复的对齐争议。
- 背景补齐**精度/dtype 配置**：前向激活/权重 FP8 E4M3（上界 448）、反向梯度 FP8 E5M2（上界 57344）、master weights/optimizer state FP32、`GradScaler(init_scale=65536, backoff=0.5, growth_interval=2000)`。依据是「scaler 在持续衰减」这一事实——BF16 梯度有 FP32 的指数范围、几乎不溢出，也就不需要 loss scaling，故反向必为窄范围格式。并标注前/反向天花板相差 128 倍，是「反向先报警 203 步、前向最后才崩」的格式层原因。
- 矫正「人话时间线解读」中关于 AMP 天花板的表述：天花板是格式规范定死的最大可表示值、检测为 isinf/isnan 二值判断（非阈值比较）；`unscale` 会在 optimizer step 前抵消 scale，故减半**不改变真实梯度与参数更新幅度**，只是为下一步腾出表示余量。
- 矫正「问题影响1」另两处表述：① `logits 变大也正常` 补上限定（值域 ±∞、靠 z-loss 约束尺度，只要不越 E4M3 的 448 即可正常算 loss）；② `梯度更新幅度每次碰顶被压缩 4 次` 改为「缩放后的梯度每 ~50 步碰顶、scale 被减半 4 次，而真实更新幅度因 unscale + Adam 归一化几乎没变，所以 loss 曲线平稳」。
- 修正 `max(logits)` 在 step 15200 的水位：原写 `~800` 已超出前向 E4M3 的 448 上界（真到 800 则 15200 就该崩，与 15203 才崩矛盾）。按 scaler 减半节奏反推的倍增周期（T_double≈53）外推为 **~410（占 448 的 92%）**，且 30→448 需约 207 步、与实际 203 步吻合。并补上 1846 的来历：趋势项吃掉余量后，step 15203 某个 micro-batch 的激活偏大完成最后一击；1846 是 **cast 成 FP8 之前**的高精度 dump 值，故可超过 448。
- 模型层新增小节「为什么定位到 layer 38，但修复不针对 layer 38」：明确 W_gate 的归属是**层级私有、层内公用（一个 `[hidden,256]` 矩阵、每列一个 expert）、rank 间复制**（这也是路由塌缩在 64 rank 上同步发生的原因）；提出「45 个独立 W_gate 赛跑、只有首个越界者有意义」的框架；给出深层跑得更快的两条机制（深层表示各向异性更强、靠近输出端梯度更强），并显式**排除**「深层激活幅度更大」这一条（router 吃 `RMSNorm(h)`，归一化已掐断该路径）；引案例四同样把路由热区定位在 layer 38~45 作为佐证；据此说明层号是定位坐标而非修复对象，故 7 条修复项均为跨层全局配置。并补 `max|router_logits|` 应逐层采样取全模型 max、同时记录 argmax 所在层的监控做法。
- 模型层表格补 **产出** 行（异常层 / 塌缩形态 / dead expert 占比 96.5% / 待追查项），与其余各层的表格结构对齐。
- 「问题埋伏」与「问题影响1」之间新增 **「同因异症」章节**（含 mermaid 图）：用一张 `graph TD` 画出「根因 → 分支 I（反向 / 预警半段）+ 分支 II（前向 / 致命半段）→ 腿 A（loss NaN）/ 腿 B（塌缩 → 死锁）→ 两个终局」的全貌，并用虚线标出 **II → I 的弱反馈**（塌缩使有非零梯度参数量 92B → 6.3B，每元素 RMS ×3.8 → 加速 I 碰顶）。配三条关系表明确方向：根因→I/II 并列 ✅、II→I 弱反馈 ✅、**I→II 不存在 ❌**。并说明这不是「硬凑两个问题」——同因异症正是误诊主要来源（看 timeout 查网络、看 NaN 查数据，二者看似分属通信与数值两个子系统），本案例的价值就在于证明它们同源；同时诚实注明分支 I 在诊断链条上并非必需，其全部价值是提供唯一的早期信号。
- 上一条新增的「并列双支框定图」随之收敛为一句指向「同因异症」的指针，避免两处重复；交叉引用改为实际标题「影响2」。
- 「问题影响1」开头新增**并列双支框定图**：分支 I（反向，梯度尾部 ×65536 > 57344 / E5M2 / 15000~15202 反复 / 可恢复）与分支 II（前向，logits > 448 / E4M3 / 15203 一次 / 不可逆）是同根因的两支，**I 是 II 的预警信号而非原因**；此前该节把两者混在一段讲、未交代关系。
- 「问题影响1」补**梯度侧的具象数值**：区分 per-element 门槛 0.875 与聚合量 grad_norm=12.4（AMP 检查前者、文档只记后者，是"溢出不具象"的根源）。按 92B 总参数 / 6B 激活参数解出共享参数 ≈3.2B、expert ≈88.8B（占 96.5%），换算每元素 RMS：正常态 4.1e-5（×65536=2.7，距上界 21403 倍）、塌缩态有效 N 降到 6.3B → RMS 1.6e-4（×65536=10.2，距上界 5622 倍）。结论三条：溢出是纯尾部事件（需离群达 RMS 的 2 万倍，故表现为每 ~50 步零星碰顶）；塌缩使每元素 RMS 放大 3.8 倍、是碰顶变频的第三个叠加因素（且 247 个 expert 梯度归零 = 96% 参数停止学习）；grad_norm 是平均型指标、found_inf 是极值型指标，这才是 12.4 全程平稳的原因 → 监控应记 `max|g|` 及其离 0.875 的余量。
- 改写「问题影响1」中 loss_scale 的表述：明确 scale 乘在 **loss 标量**上、经反向线性性等效放大全图梯度（非逐个乘梯度、也不乘 grad_norm），本意是**防下溢**而非放大；新增 loss / 梯度对照表解释「同样 ×65536 为何只有梯度越界」——缩放后的 loss ≈ 20 万住在 FP32（余量 1.7e33 倍，且比梯度天花板 57344 还大 3.5 倍），梯度住在 E5M2（真实梯度超 0.875 即溢出）、有数百亿元素、分布重尾，且 AMP 只检查梯度。并注明本案例 loss 全程未越界，其 NaN 是经 `mean` 被前向污染所致。


## 2026-09-11 — 平面视图·专家负载：复现 E193 路由塌缩事故

- “负载热力 → 专家负载”将 openPangu EP64 的塌缩点固定到事故记录中的 Layer 38 / Expert 193 / EP rank 23，不再由通用热度模型挑选热点。
- 末相按原始统计展示 Expert 193 获得 8028/8192 token（98.0%）、247 个 dead experts，以及 rank 23 的 All-to-All `send=0 / recv=9832`；悬浮提示、顶部事故横幅和右侧负载详情使用同一份数据。
- 右侧负载详情复用配置寻优格内的 `Expert Compute` 专家胶囊，按选中格的 EP rank 只展示本地专家分片（例如 rank 108 × Layer 38 的 E192–E223），而非全层 256 expert 总览；矩阵与路由时间轴同拍，沿既有冷蓝→火红色阶展示 E193 汇集 token、同格其余专家冷却的过程。
...
- **标题字号加 4px**：原先写的 `var(--font-size-label-sm)` 是个全仓没定义的 token，声明失效、实际吃的是 h3 的 UA 字号（12px × 1.17 ≈ 14px = body-md 那一档），现在明确写成 `calc(var(--font-size-body-md) + 4px)` = 18px。

---

## 2026-09-08 — 平面视图·集群矩阵：格内从两档扩到三档，最细一档铺出 rank 内的 Attention / MoE 计算节点

- **格内内容改成三档，判据全部换成「这一格在屏幕上有多大」**（`js/config-relation-plane.js` 的 `BLOCK_MIN_H` / `DETAIL_MIN_S`）：① 整格上色 → ② Hidden / Attention / MoE 三段块 → ③ 逐个计算节点。原先是两档、由 `k >= 0.5` 一个数决定，而同一个 k 下每卡 32 个专家的格子有 139px 高、每卡 4 个的只有 22px，两者能写下的东西差着一个数量级。缩放上界随之从 4 抬到 16，否则后一类配置永远进不到最细档。
- **最细档铺出这张卡在这一层里真正要跑的算子**（`buildDetail`）：Attention 组（Input RMSNorm / Q·KV LayerNorm / Q·KV Up Linear / Sparse FlashAttention / Output Projection）→ Residual Add → MoE 组（Router / EP Dispatch / Expert Compute / EP Combine）或 Dense FFN 组 → Residual Add。id 与文案照抄 `patterns/model-architecture-3d-deck` 那张「典型 Layer」卡，平面里的一个算子与整网图、结构条里的同一个算子必定同名。面板按一套设计像素排版再整体 scale 进格子，高度由 js 的 `panelH()` 与 css 的盒模型同源算出。
- **整幅平面上唯一带颜色的东西是专家**：算子块、组框、Hidden / Attention / Dense 三段块里的非 MoE 那几条，一律中性灰的深浅，不取 deck 的 `data-op` 语义色。颜色在这幅图里已经被占用了 —— 它说的是「这一片属于哪一套专家」，再叠一套算子色码，读的人分不清哪一片绿说的是哪件事，先被冲掉的正是专家那一套。算子之间的区别由文案与位置说。
- **最细档整幅共用一个缩放比、一个档口**（`detailTallest` / `detailS`）：比例按最高的那种面板算一次，dense 列与端点列共用。原先各算各的，dense 面板矮、比例大，于是比 MoE 早一截进档 —— 中间那段缩放里同一屏上 dense 列已是计算图、MoE 列还是三段块，看着像是两种层的区别，其实只是两个门槛。
- **专家编号从「铺满整格」缩进 Expert Compute 盒子**：编号仍可点（`data-kind="expert"` 不变，pick 顺着父格子取坐标），但它现在是路由链上的一环，不再是整格的全部。中档由 MoE 那一条带接住 `--crop-set`，「同色一块 = 一整套专家」这条读法穿过档口不断。
- **撤掉 `NODE_BUDGET` 与 `--crop-chip-w` / `--crop-chips-w`**：那条总节点闸是为「胶囊铺满整格、只受 k 一条约束」那一版兜底的；三档全部按屏幕尺寸开闸之后，一屏能有多少格子由闸门自己封住，再算一遍总数是白算。

## 2026-09-08 — 平面视图·通信观测：浮卡两块共用一副两列骨架，数据类别标签加对比度

- **浮卡的头一行与五条读数并到同一副两列上**（`.crop-comm__card` 做成两列网格，头一行与读数区用 subgrid 借这两列；不支持 subgrid 的内核退到定宽近似）：标题的左缘与 value 的左缘对齐，阶段胶囊与 key 那一列等宽、同左缘。原先两块各排一个自己的网格，列宽由各自内容决定，卡上于是有两条彼此错开的左缘。只有标题、没有胶囊的那种卡（「这一列不产生跨卡通信」）标题铺满整行，不跟着往右缩进。
- **数据类别标签（`activation` / `gradient` / `optimizer`）加一档对比度**：原先底 surface-2、字 secondary，在毛玻璃卡面上几乎和卡底一个色，读成一段灰字而不是一枚标签。改成底 surface-4、边框 border-default、字给足并抬成大写。加的是对比度不是色相 —— 卡上带颜色的仍旧只有链路那一枚。

---

## 2026-09-08 — 平面视图·通信观测：改名、工具行收紧、点格子按当前阶段续播

- **「通信查询」页签改名「通信观测」**：这一档给的是一趟能跟着走的行程，不是一次一问一答的查询。
- **中栏工具行（`.crop-toolbar`）收紧**：上下 margin 统一 8px，页签与右侧那组工具的 gap 从 24px 降到 12px（`.crop-heat-extra` 同档），页签行不再比它承载的内容高出一截。
- **页签右侧加一句口径说明（通信观测档才露）**：「默认以第一行的单个 rank 作为通信故事主角，可手动切换」—— 整趟行程只跟一张卡走是一个选择，不说出来会被读成「整个集群都这么通信」。
- **MoE 舞台的目的卡描边跟着负载色走**：原先描边染链路（机内 / 机间）、底色染负载，一张卡上两套编码；现在框与底同色阶、同一个值，只是描边实一档，机内 / 机间挪到描边的虚实上（机外虚线）。
- **通信观测档点画布上的一格，从当前阶段的这一格续播**：原先一律回到前向那一趟。正在看反向的人点一格，问的是「反向走到这儿是什么样」，把行程甩回前向等于换了个问题；该阶段不落在列上（更新那一段整块一拍）时退到这一阶段里这张卡的第一拍。

---

## 2026-09-08 — 平面视图·通信查询：反向补出跨 EDP 的专家梯度桶

- **DP=16 的同步 / 更新通信组不再漏画 rank**：画布现在完整框出锚点之外的 15 个对端；更大的 DP 域才做等距抽样，避免用户把视觉抽样误读成真实通信成员缺失。
- **每个 MoE 层的反向在 Dispatch 回程后新增一拍 `EDP Reduce-Scatter（梯度桶）`**：EP 内的输入梯度回到 token rank 后，本层专家参数梯度已经就绪；持有同一专家分片的 rank 随即沿 EDP 轴聚合梯度。画布因此第一次明确画出“各 EDP 独立反向，但梯度就绪后会跨 EDP 同步”，不再让整段反向看起来永远封闭在一个 EDP 内。
- **梯度累积口径写进频次**：`micro_batch_num > 1` 时，前面的 micro-batch 只在本地累积，只有最后一个 micro-batch 发起跨 EDP 通信；浮卡会按当前配置写出前面有几份只累积。播放入口也改称“一个 step 里的最后一个 micro-batch”，避免把这条同步误读成每个 micro-batch 都会发生。
- **原“更新”阶段的 Routed Expert EDP 同步不再重复播放**：专家梯度桶已经在反向就绪点启动；后段保留非专家权重的 DP 同步与分片参数 All-Gather，并更名为“同步 / 更新”。这既保留更新前必须等同步完成的逻辑屏障，也表达真实运行里通信会与更前面层的反向计算重叠。

---

## 2026-09-08 — 平面视图·配置评估续：单卡容量收进 L-B、静息态去掉典型 Layer、段名分两行

- **单卡容量做成可收起卡片，挂在 L-B「最紧的卡」正下方**：那一行给的是「78%」这个结论，六段柱子给的是那 78% 的构成 —— 原先两者一个在评估栏、一个在右栏底部，读的人得先记住数字再滚下去找柱子。默认收起（它是「点开看构成」，不是第四层判据）。节点仍是原版那一个 `#croCapacity`，选中态照旧搬回右栏底部与「典型 Layer」并列，不复制第二份（同页两处各报一个显存数是这一页最不能出的错）。
- **静息态（当前配置评估）不再摆「典型 Layer」**：那一条答的是「你选中的这张卡上一层长什么样」，没有选择时既没有 is-related 也没有 is-selected，只是一张与全局判定无关的静态结构图，还占着小半屏。
- **进「配置寻优」页签默认回到当前配置评估**：这一档问的是整份配置的事，换档回来第一眼不该停在上一趟调查剩下的那个对象上。切档时清空选择并展开右栏（用户手动收起过的除外）。
- **典型 Layer 的端点段名分两行**：`Emb（PP0 · L0~L11）` 在右栏那种几十像素宽的一列里必被截成「Emb（PP0 …」，而括号里那半句正是最要紧的信息。拆成上下两行后括号也不必要了 —— 第二行本来就只可能是第一行的定语。
- **左栏 AI 输入框改蓝紫渐变描边**：两层背景（padding-box 铺面色、border-box 铺渐变）实现，全页唯一一处渐变，标的正是「这里接的不是表单控件，是模型」。落焦只加光晕不换色。

---

## 2026-09-08 — 平面视图·配置评估：判定做成语义卡、四层做成巡检清单，左栏底部加 AI 输入框

- **总判定升格成语义卡**：「预计可跑」抬到 title-sm 并带同色状态点，可跑档整卡转绿（原先只有偏紧的黄、越界的红有底色，绿档反而是唯一读不出语义的一档）。下方新增一根占用尺，把判定的凭据画出来 —— 最紧那张卡装到几成、离 70% 波动线还有多远。判定的颜色于是有个看得见的来处，不再是读完整段文字才成立的断言。
- **每行指标行首加一枚巡检色点**（绿通过 / 黄偏紧 / 红越界 / 灰未知）：整栏纵向扫一遍就是一份 checklist，不必逐行读数字才看得出这一层有没有事。灰点做成空心 —— 既不是通过也不是不通过，实心会读成「有个结论」。
- **需要真跑一遍才知道的几行不写结果**：L-C 整层压暗一档（它要回答的正题「跑得多快」本页给不出），其中「重计算算力开销」与「吞吐 / step time」标成「待实跑」、不摆占位数 —— 和旁边那些算得出来的数长成同一副样子就是在冒充已知。经验系数折算值仍在问号气泡里查得到。
- **L-A~L-D 每层那段导读搬进标题右侧的问号**：四段小字摞起来会让评估栏第一屏全是解释、一个读数都看不见。走页面统一的 `data-hint` 气泡（与 MoE 那节标题同一套触发）。
- **左栏底部浮一枚 AI 配置输入框**（外观件，功能待接入）：占位语是一句真能提的需求「改成 1024 张 32G 的卡，请给出推荐配置方案」，一眼就知道这里接的是整份配置的改写而不是搜索。绝对定位浮在配置流之上而不是排在表单末尾 —— 左栏能滚很长，排末尾等于拨到底才看得见。已把 `.crop-ai` 加进 `config-relation-observer.js` 的 SELECTABLE 白名单，否则一落焦就把当前选中对象清掉。

---

## 2026-09-08 — 平面视图·通信查询：浮卡加一台 MoE 微观舞台（router → dispatch → combine）

- **MoE 那三条通信（Router 打分 / Dispatch / Combine）在浮卡里多出一台小舞台**：画布上的连线答的是「哪几张卡在同一个 EP 域里」，答不了 all-to-all 里面在搬什么 —— 而它其实是三件事的合成：router 在卡内逐 token 打分取 top-k（零通信）、dispatch 按目的地重排发出去、combine 按原次序收回来。舞台上排是抽样的 16 个 token，下排是这一拍要发往的那几张卡，中间是**当前那个 token**自己的 top-k 连线。
- **焦点 token 一拍之内轮完 16 个**：top-k 是 per-token 的，一次把 16×k 条线全画出来只会糊成一片；一个一个亮过去，「每个 token 各自选自己的 k 个专家」这件事才看得出来。dispatch 的动点从 token 飞向目的卡，combine 反着飞，Router 那条不给动点（打分在卡内完成）。
- **专家分布按「按层定种子的亲和度」算**（口径与 `ParallelDemo/dist.html` 的 `_moeAffinity` / `_moePickTok` 同源）：同一层各卡共用一份 router 权重、偏好一致，每个 token 再叠自己的噪声 —— 于是各专家收到的 token 数天然不均。目的卡底下那条实心的长短就是这个不均，它是 MoE 最关键的性能事实（负载均衡 aux loss 与 capacity factor 要治的正是它）。
- **卡底那条随 router 打分逐格长出来**：16 个 token 一个一个出结果，各卡的累计条按前缀和往上涨；颜色不再用链路色，改成复用「负载热力」那条色阶（heatColor，量纲是相对最热的那张卡）—— 一开始各卡同为最冷色，谁被选得多谁先走向暖色，负载不均是**长出来**的而不是一上来就摆好的设定。dispatch / combine 两拍直接用最终值（那时路由早已定完，再慢慢长会读成「正在决定」）。整张目的卡也跟着同一个值淡染（moeHeatFill，同色阶低 alpha），卡与卡底那条因此是同一件事的两种强度 —— 一个按热力涨、另一个按链路染会被读成两套无关的编码。机内 / 机间那一档因此只留在**描边**上：它是这张卡的固有属性、不随打分变，正适合放在不变的那一层。
- 目的卡按收到的 token 数排序，超出的**不丢掉**，折成末格「+N 张」：丢掉的话标题说「每个 token 选 8 个专家」、画面上却只有两三条线，正好把这台舞台最该说清的事说反了。目的卡的机内 / 机间用 `--crop-link-*` 染色，与画布连线、链路图例同一套色。
- 舞台标题写真实 token 数：`microBatch × (seqLen / cp)`，开了 SP 再除以 TP —— 与 `buildHeat` 里的 `tokens` 同一口径。脚注给 top-k 守恒式：`16 个 token × top-k = N 次激活，落在 M 张卡上（其中 X 张在机外）`。

## 2026-09-08 — 平面视图·通信查询：浮卡改两列网格、加数据类别标签、传输内容补「为什么传」

- **五条读数改成两列网格**（`grid-template-columns: max-content minmax(0,1fr)`，`.crop-comm__kv` 用 `display:contents` 把 key/value 直接交给网格）：key 一列、value 一列，各自左对齐。原先是长的独占一行、短的两两并排，每一行的 key 都落在不同位置上，扫下来要重新找一次左缘。
- **卡顶新增一行标签**：链路徽标（机内 HCCS / 机内+机间 / 机间 RDMA）从标题行挪出来独占一行，右侧并排一枚中性的**数据类别**标签 —— `activation` / `gradient` / `optimizer`。两枚都是分类读数，和标题混在一行会被读成标题的一部分；而「传的是哪一类」还决定了这条通信的量随什么变（激活跟 batch×seq 走、梯度跟参数量走、优化器状态跟切分档走）。路由的负载统计两类都不沾，就不给标签。
- **传输内容补上「为什么非传不可」**：每条事件新增 `why`，浮卡里渲染成「传 xx —— 以便 xx」。只写「传部分和」答不了这条通信为什么存在，而那才是并行策略贵在哪儿的解释。
- **更新阶段补一条「参数 All-Gather」**（`shardMode` 为 ZeRO-1 / FSDP2 且有 DP 副本时才成立）：每张卡只更新自己那一片参数，下一个 step 的前向要用完整权重，得先拼回来 —— 这正是「切分省显存、代价是多一趟通信」那笔账，也是 optimizer 这一类标签的唯一来源。不切分（`shardMode: none`）时整条不出现。

## 2026-09-08 — 平面视图·负载热力：五个度量各埋一处典型故障，画布顶上加红横幅点题

- **位置那一半的抖动整体加宽**：算力抖动从「整机 ±3%」拆成「整机 ±8% × 每卡 ±3%」（供电散热是整机粒度，个体差异是单卡粒度），HCCS ±6%→±12%，RDMA ±15%→±25%；专家人格表 `expertBase` 区间从 [0.45, 2.00] 放到 [0.30, 2.20]。一幅处处 0.98~1.02 的热力图读不出任何东西，而真实 router 的不均本来就比那大得多。
- **`expertBase` 收成唯一一份**：前五个度量的路由不均（`skewOf`）与「专家负载」那一档的分布（`routeOf`）此前各写各的常数，同一份配置会在两处指出两批不同的热门专家 —— 两幅图互相矛盾。现在同读一张人格表。
- **五处刻意构造的典型故障**（`FAULT`，选点走 `hash01` 不是随机数，同一份配置每次落在同一批卡上，可指名道姓地查；配置一拨跟着换一批）：
  - 内存占用 — 某个 EP rank 上热门专家扎堆，permute 暂存 ×2.4 → **一条横带**，全图最先触顶的一批卡（与运行事件问题1 的 EP rank 碎片 OOM 同源）
  - 计算量 — 某一段 PP stage 的层重 34%，PP 却是按层数均分的 → **一整块列区**，切分不均
  - 计算时间 — 一台机器算力只剩 68%（降频 / ECC 退化）→ **一条横跨所有列的整机带**，「不挑列」正是它与「某一层重」的区别
  - 空闲时间 — 同段的卡等全额、隔段等半份 → 与计算时间**互补**：慢卡自己是全图唯一一条暗带（它不等人）；另外把 PP 气泡改成沿 stage 的阶梯（末段暖机等得最久）
  - 通信时间 — 一个机架的 RDMA 只剩 42%（上联光模块劣化）→ **连续几条整机带**，且只打在跨机那几条通信上，机内 HCCS 不受影响
- **色阶采样补上故障行**：`allRanges` 的采样点显式加入慢卡整机、坏机架两端、触顶 EP rank 那几行 —— 漏采极值会让色阶上端顶不到那条带子，整幅图看起来「哪儿都差不多」，而那条带子恰恰是这张图唯一要说的话。
- **画布顶上加一条红横幅**（`.crop-heatbanner`）：把当前这张图埋的故障翻译成一句人话并指名道姓（「整机 37 降频到 68%」而不是「有一台机器慢」）。悬浮不占布局高度、让开顶部量尺、`pointer-events:none`（是注解不是控件，因此也不给关闭键）。「专家负载」那一档的横幅随时间轴走，写的是当前那一相与塌缩点。
- 格子气泡同步补一行「踩到的是哪处故障」：没有这一行，那条带子只是「这里比较烫」，读的人还得自己猜是什么烫了它 —— 而这幅图的价值就在于把形状翻译成毛病。

## 2026-09-08 — 平面视图·负载热力：第六个度量「专家负载」，带一根路由塌缩时间轴

- **新增度量「专家负载」**（`HEAT_METRICS` 的 `route`）：格子答的是「这张卡在这一层收到的 token 是均分的多少倍」，1.00× 为理想均衡。它与另外五个度量不同类 —— 那五个由配置直接推得、与时间无关，这一个多一根**时间轴**，复刻运行事件 2.5（`p1-root`「Router FP8 溢出，E193 吸收 98% token」）：事件那边给的是一层、一个 step 的静态截面（98% / 8 个活跃 / 247 个 dead 三根柱），这里把它摊回整幅平面 + 一根进度轴。
- **四相时间轴**（`ROUTE_PHASES`，工具带上播放键 + 滑杆 + 相位读数）：① 正常路由 → ② 分布走偏 → ③ logits 越界 → ④ 路由塌缩。前三相是 α 锐化（每个专家的基础热度取 α 次方再归一，α 随 τ 从 1 涨到 6）演的**慢性**偏斜：热点卡从 1.3× 长到 4.2×、位置不动；④ 才向 one-hot 混合，热点卡跳到 62.8×（= 98% token），同层其余落到 0.04× 全灭。②③ 在图上**故意长得一样** —— 越界只发生在数值里，热力图看不出来，那正是这次事故最难查的地方，差别只由相位读数那行字说出。
- **塌缩点由配置定死、可复现**：按「锐化到底时哪一格最重」挑层与 EP rank，再在那一格里挑最受宠的专家。所以吃掉一切的那个专家，就是慢性阶段里本来就最热的那个 —— 换个挑法（比如按纯人格哈希挑）会让前三相烫 A 卡、第四相塌在 B 卡上，读出「塌缩不可预警」这个错结论。
- **非 MoE 列留素底**：Dense 层与 Emb / Norm / Head 没有路由，取值给 `NaN`、不写 `data-heat` —— 涂成色阶最冷那一端等于说「这里的专家被饿死了」，而真相是这一列压根没有路由。色阶两端也改成实测（不再把下端钉在 0），否则 τ=0 那一档的全部内容「1× 上下那点起伏」会被整片推进暖区。
- **切到「专家负载」自动从头演一遍**（`startRouteDemo`）：另五个度量切过去就是一张成品图，这一档若停在 τ=0 看到的是一片均衡 —— 那正是「什么都没发生」的样子，读的人会以为这一档坏了。塌缩是这一档的全部内容，它得自己演出来。播放键的正职因此变成**重播 / 暂停**。切进这一档一律归零重来（不接着上次的位置），否则前三相那段慢性偏斜会被跳过去，而那半正是要讲的东西。
- 色阶量程随 τ 变，因此不走 `allRanges` 那份按配置缓存的采样表（`HEAT_SCALED` 把它排除），改由 `routeRange` 直接扫塌缩层 + 若干代表层的 EP 分布算出来 —— 便宜两个数量级，且极值一定被算到。

## 2026-09-08 — 平面视图·通信查询：事件行只列本列真发生的、链路色避开专家四色

- **事件行左侧加列名标题**（`.crop-comm__row-label`：`Layer 13` / `Emb` / `Stage 1 全部参数`），并**只列当前这一列真会发生的那几条**：Dense 层没有 MoE 的两次 All-to-All、段中间的层没有 PP 收发 —— 把不发生的也摆出来，这一行就从「现在在做什么」退回成一张不随位置变的清单。一条也不发生的列（Emb / Head、并行度为 1 的 Dense 层）给一句「这一列没有跨卡通信」占位。
- **事件行与阶段带左对齐**：`.crop-comm__rows` 补 `padding-left: calc(36px + var(--space-3))`，让开播放键那一格 —— 否则「这一阶段的进度」与「这一阶段正在发生什么」看起来像两块没关系的东西。
- **事件条撤掉第二行**（模块 · 范围），整条从 32px 收到 24px：那两项浮卡里写得更全（还带传输内容与频次），而这一行每换一拍就重排一次，两行会让它在画布上方晃得很厉害。同名事件（一层里 Attention 与 MLP 各有一次 TP All-Reduce）由 js 现补模块名，不为少数情况常驻一整行。
- **链路三档换色，避开专家那四色**（新增 `--crop-link-local/mixed/inter` = 青 #06B6D4 / 品红 #D946EF / 绯红 #E11D48）：连线与图例是画在专家色块**上面**的，原先用 success/warning/primary 正好撞上专家的绿/琥珀/蓝，两套说的是完全不同的两件事（「属于哪一套专家」vs「这条通信出不出机器」），撞色等于把它们读成同一件事。新三色同时也是一条「越往右越贵」的成本坡。
- **通信查询档下点画布上的格子 = 从这一格往右开播**，不再发出选择、也不再顶开右栏：这一档的主体是那条时间轴，右栏那份静态档案（这张卡是谁、持有哪些专家）在这里既答非所问、又要抢掉画布近三分之一的宽度。行程认的是「行」，所以点到别的行会整趟重排，落点取前向那一趟里这一列的第一拍（一格上可能有好几拍）。行标、量尺、stage 带照旧发选择。
- **一个 step 走完自动绕回第一拍接着播**（原先停在末拍）：停在末拍时那一拍的动点还在原地循环，看起来和正常播放的一拍一模一样、只是永远不往下走，读成「这一步要等很久」。训练本来也是一个 step 接一个 step，绕回去正是它的样子；要停由用户按暂停。
- **对端那几格也加白框**（`.crop-commflow__cell--peer`）：比主角细一半、透一档，并用一层极轻投影兜住浅底 —— 主次一眼分得出，同时答得出「通信的另一头是**哪一格**」（原先只有一个落在格子中间的小圆点）。

## 2026-09-08 — 平面视图·通信查询：图例移入页签行、阶段带等宽标层、更新阶段改框整段

- **链路图例移到页签那一行的最右端**（新 `.crop-comm-extra`，与热力档的 `.crop-heat-extra` 同一位置，`margin-left:auto` 顶到右侧；`.crop-toolbar` 相应改 `align-self:stretch` 才顶得出去）：它是这一档的常量（三档链路各是什么颜色），不随播放变，摆在带子里会和进度带抢那条本该整条留给「现在演到哪」的横向空间。
- **阶段带三段等宽**（`flex:1 1 0`，不再按拍数分宽）：更新只有几拍，按拍数分会被压成一条既读不出字也点不中的缝，而这条带子的第一职责是「现在演到哪一阶段、点哪儿能跳过去」。段内进度仍然是真的（`--prog` 涨到这一段的第几拍）。
- **进行中的那一段把当前位置写进段名**：「前向 · Layer 13」「更新 · Stage 1 全部参数」—— 段名是这条带子上唯一能一直写字的地方，而「演到哪一层」正是播放时每一拍都在变的那个读数。
- **修：更新阶段原先落在块中段那一列**（12 层的段就是 Layer 6），看起来成了「只有 Layer 6 在同步」，纯属摆错位置 —— 它同步的是这张卡上**全部**参数的梯度，不属于任何一层。改成一段一拍、白框铺满**整块**（拍上新增 `wide`，`flowCellRect` 与顶部量尺的 `is-flowing` 一并按它铺整段），段与段之间仍按左到右各走一遍（每一段的卡都各自要同步一次）。
- 播放速度整体放慢一半：整趟目标时长 60s → 120s，单拍上下界 320/1100ms → 640/2200ms（三个数是同一档速度的三个面，要调一起按比例调）。

## 2026-09-08 — 平面视图·通信查询：阶段进度带取代两行文字，事件条只列当前阶段

- **播放键右侧的标题 + 读数两行文字整块撤掉**，换成一条**阶段进度带**（`.crop-comm__phases`）：借 `training-monitoring-v2` 的 L2 阶段带（`.tw-pano__phases`）那副语言，只把蓝换成本页主色 —— 宽度 = 这一段占整趟行程多少拍，实色是走过的部分、45° 斜线是没走到的，进行中那一段在交界处补一道竖线（`--prog` 每帧只改值不重建 DOM，与 `tickProgress` 同一手法）。原来那两行说的东西浮卡里都有，却常驻占着一整条带子高度。
- **进度带同时是选择器**：点前向 / 反向 / 更新任一段，就从那一段的第一拍开演 —— 不必把前面几十拍等完。
- **`crop-comm__rows` 从三行减到一行**：只列当前阶段的事件条（阶段名也不再写在行首，进度带已经标出来了），画布拿回近三分之一高度；三个阶段本就是先后发生的，同屏摆三行还容易被读成「同时在跑」。
- **行程从「只有前向」补成一整个 step**：前向从左往右扫 → 反向整个掉头（块倒着走、块内的列也倒着走）→ 更新（梯度同步不属于任何一层，每条各占一拍，落在第一段中段一列）。行程改为由 `renderComm` 常驻维护（进度带要按它分宽标进度），不再只在按播放时才排。
- **一拍的时长改成按总时长反推**：`clamp(60s / 总拍数, 320ms, 1100ms)`。原先固定 1s 一拍，46 层的配置要看两分多钟；现在整趟恒定在一分钟上下，深模型自动走快、小模型自动走慢，下限保证动点还看得清方向。
- 阶段名统一叫「更新」（原「梯度同步」），与进度带、v2 面板同名。

## 2026-09-08 — 平面视图：通信 / 热力档默认 Rank 视角，行程主角格加白框

- **进「通信查询」「负载热力」两档时把画布粒度默认切到 Rank 视角**（`setMode` 里写 `unitMode = "rank"`）：这两档答的都是「**这张卡**在这一层上怎么样」，自适应档一缩小就把 8 张卡聚成一行，那个问题当场问不出来（行程的主角落在整机行里、热力那一格是 8 张卡的平均）。默认只是默认 —— 粒度那一组按钮照常可点，切回自适应 / 整机后本档内不再被顶回来，下次进这一档才重置。
- **行程主角那一格加白描边**（`.crop-commflow__cell` + `__cell-halo`，画在连线那一层 svg 里，每帧现算屏幕坐标）：白线在浅底与专家浅色块上会糊掉，所以底下垫一条半透明深色粗框当光晕，两条同心矩形合起来读作一圈带光晕的白框；两条都不填充，格子里的专家编号与热力色照常可读。整机视角下框住的是整行合成的那一格（行首对齐到粒度边界），不会横穿一条整机行。
- 白框**不依赖这一拍有没有通信**：扫到 Emb / Head、并行度为 1 的 Dense 层时连线为空，但「现在演的是哪张卡的哪一格」照样要答得出来 —— 连线层因此改成只要有拍就亮，起点圆点则只在真有线时才建。

## 2026-09-08 — 平面视图·通信查询：行程改成整幅平面从 Emb 扫到 Head

- **默认锚点回到第一行（rank 0），行程沿同一行把整幅平面从左扫到右**：Emb → 第一段每层 → 出段 → 下一段（行号相同的那张卡）→ … → Norm → Head，默认配置下即 rank 0 → 32 → 64 → 96。拍从 (rank, layer, event) 扩成带列坐标的 (rank, stage, ci, layer/unit, event)：Emb / Norm / Head 不是层却同样是一列，并行度为 1 的 Dense 层也一样 —— 这些列没有跨卡通信，仍各占一拍（浮卡改说「这一列不产生跨卡通信」），行程才不会断档。
- **补上 Dense 层 FFN 的 TP All-Reduce（前向 + 反向各一条）**：一层里本就有两次 TP All-Reduce（attention 出口 + FFN 出口），原清单只列了 attention 那次，Dense 层在行程里只剩一条通信，与实际差一半。MoE 层的 FFN 走专家那条路，所以新条目与 MoE 那几条按层互斥（`flowApplies` 同时管 MoE-only 与 Dense-only 两侧）。
- **进 / 出 stage 两条按段边界过滤**：第一段没有上游可收（不排 PP Recv）、最后一段没有下游可发（不排 PP Send）。
- **反向那一行改成从右往左排**（`row-reverse` + `justify-content:flex-end`，整组仍贴左缘）：反向的第一条紧接前向的最后一条，从左排会和箭头方向打架。
- **播放时的三处高亮**：带子里正在演的那一条加 `.is-playing`（提饱和度 + 轻微明暗呼吸，不用描边/投影 —— 那两样会被箭头的 `clip-path` 裁成断口）；顶部量尺对应的那一列加 `.is-flowing`（主色实底 + 底部 2px），疏刻度档下也强制留出这一格；只重铺量尺不走 `scheduleRender`（后者要重建几千个格子，每秒一次会抖）。
- **顶部量尺：列宽写不下全名时改用短名**（`Layer3` → `L3`，阈值 `cellW·k < RULER_COL_MIN+12`），省下的宽度留给第二行的 Dense/MoE。
- **PP stage 带不再跟随层/卡的选择变色**（去掉 `is-related` 那一支）：选一层时它所在的段必然被牵连，整条段带几乎永远亮着，信息量为零却把这一层从刻度变成一片乱闪的底色；只有直接点中这一段才给选中态。
- 在画布上点中另一张卡：停播、锚点跟过去，并落在**那张卡所在段**的第一拍（而不是把画面拉回 Emb）。

## 2026-09-08 — 平面视图·通信查询：单 step 通信动画（连线 + 动点）

- **播放键接上**（`crop-comm__play` 不再置灰）：按下后沿行程一拍一拍往后推（每拍 1s），当前这条自动滚进视野；再按一次暂停，走完自停。手点任意一条 = 接管时间轴（暂停并只看那一条）。
- **画布上新增一层连线（`.crop-commflow`，svg，压在两条量尺之下）**：以当前选中的卡为锚（未选中取 rank 0），把这一条通信的「谁和谁」画成弯曲连线 —— 线上的动点表示数据方向（PP Send/Recv 单向、All-to-All 双向、All-Reduce 先聚后散两趟、CP Ring 只连环上的下一个邻居）。落点按事件所在位置取列：进/出 stage 落在块的两端（线因此横跨在两个 stage 块中间的空当上），MoE 的 All-to-All 落在第一个 MoE 层，其余取块中段一层作代表。
- **连线颜色逐条判机内/机间**（绿 = 同一台机器的 HCCS，蓝 = 跨机 RDMA），而不是用整组的 `link.kind` 一刀切 —— 带子上的「mixed」在这一层自然表现为一束里绿蓝混着，正是它的字面意思。域大于 10 个成员时等距抽样 10 条，既留近邻也留最远的一端。
- 坐标每帧现算（`flowDraw` 走 rAF），播放期间照常缩放平移，线钉在格子上；离开通信档 / 改配置自动停播并清线。
- 顺带修：点带子里的事件条会冒泡到 `SELECTABLE` 那条兜底、把当前选中的卡清掉 —— 而这一档的「谁和谁」正是以那张卡为锚。已按热力胶囊同样的做法 `stopPropagation`。
- 修：连线基本看不见 —— svg 是**替换元素**，`position:absolute; inset:0` 下 `width:auto` 取的是它的固有尺寸（300×150），`inset` 拉不开它，于是只有落在左上角那一小块里的线画得出来，其余被 svg 自己的视口裁掉。`.crop-commflow` 显式写 `width/height:100%`。
- **时间轴从「事件清单从头念到尾」改成「逐层推进」**：清单里的前向事件除进出 stage 那两条外，每一层都要来一遍，所以一个「拍」是三元组 (rank, layer, event)，播放沿横轴一列一列往右推 —— 进 stage → 本段每一层（Dense 层只有 TP/CP，MoE 层多出 Router + 两次 All-to-All + 共享专家）→ 出 stage → 激活交给下一段里行号相同的那张卡 → 演它的第一层。默认配置下即 rank32 走 Layer12–23，再跳到 rank64 的 Layer24。每拍 1s。
- **连线的落点跟着拍的层号走**（不再取"块中段一层"作代表）：PP 仍按「早的那头画块尾、晚的那头画块首」压过层号，所以那一跳横跨在两个 stage 块中间的空当上。
- **默认锚点不再是 rank 0，改取 stage 1 的首卡**：rank 0 在第一段，前向开头是 Emb 而不是 PP Recv，看不到「上一段把激活交过来」这一跳。进「通信查询」档即自动摆好行程第一拍（打开就在动，不必先点）；在画布上点中另一张卡则停播、锚点跟过去、从那张卡所在段的第一层重开。
- 带子的选中态、浮卡、连线统一由 `flowBeat` 一处驱动（原先 `commSel` 那份独立状态取消），浮卡标题与带子说明补上「Layer N · rank N」与行程进度。


## 2026-09-08 — 平面视图：负载热力的胶囊 / 图例 / 说明并入页签行

- **度量胶囊（`crop-heat__pills`）与色阶图例（`crop-heat__legend`）从画布上方的独立工具带，改摆到三档页签（`crop-modes`）右侧同一行**（新增 `.crop-toolbar` 包一层，`.crop-heat-extra` 装胶囊 + 图例 + 问号），页签与胶囊、胶囊与图例之间各加一条竖分隔线、留 24px（`--space-6`）间距；热力那半只在选中「负载热力」时露出，画布因此拿回了原先被独立一整条工具带占掉的高度。
- **五枚胶囊各自的悬浮说明 + 原先常驻的脚注文案，合并成一枚「?」**（`crop-heat__help`，摆在这一行最右侧），复用 `config-relation-observer.js` 现成的 `cro-hint` 悬浮气泡机制（`HEAT_HINT_TEXT` 一次拼好五句度量解释 + 数据构造方法论），不必在本文件另起一套弹层；点它也不会误清空当前选择（该机制已在 `SELECTABLE` 判定之前放行 `.cro-hint`）。
- 移除 `.crop-heat`（画布上方独立工具带容器）与 `.crop-heat__note`（常驻脚注），`syncHeat()` 相应精简 —— 图例两端仍随度量/配置现写，脚注不再逐度量重写（已是静态文案）。

## 2026-09-08 — 配置关系观测：cinnnnnndy 配置改为打开时的默认档

- **`CONFIG_PRESETS["openpangu-flash"]` 把 cinnnnnndy 排到首位**。`defaultConfigPresetId` 取的就是数组首元素，所以首位即「这一页打开时用哪一档」。理由是这一页最常被用来看这份 128 卡的实跑配置，而 2048 卡的参考配置一屏铺不完，开局要缩到 6% 才看得见全貌。原第一档保留为第二档并改名「openPangu 2.0 参考配置（2048 卡）」—— 它是 `overrides: null` 的那一档（= 模型自己的 defaults），必须留着当回头路。
- **首帧的种子改走新的 `configSeed(modelId, presetId)`**（`createController`）。原先首帧直接摊 `MODEL_PRESETS[modelId].defaults`，而 `config.configPreset` 却写的是首档预设的 id —— 首档一旦不是「模型自己的 defaults」，下拉就会写着 cinnnnnndy、页面上的数却是 2048 卡那一份。`configSeed` 是「一档预设展开是什么」的**唯一**来源，首帧种子 / 换模型 / 换预设三处共用，不再各拼各的。
- **首帧补算两个派生字段**：`totalRank = parallelWorld(config)`、`node = nodeLayout(config).node`。它们平时由 `reconcile` 末尾回写，而首帧没人跑过 reconcile（`refresh` 就是 `emit`）—— 盖上 overrides 之后不补这一下，Total Rank 那格会显示 defaults 里的 2048，与 16×4×2×1=128 的几何对不上。`setModel` 里同样补一次（它原先只补了 node，因为那时摊的一定是自洽的 defaults）。
- 超规模软警告的参照系**不变**：`warn()` 读的仍是 `MODEL_PRESETS.defaults.totalRank`（2048），与 CONFIG_PRESETS 的排序无关。

## 2026-09-08 — 通信查询：详情卡改成画布左下角的浮卡，不再占带子高度

- **详情卡从带子里搬到 `.crop-stage` 内，绝对定位在画布左下角。** 原先它长在带子末尾，点开一条就把画布往下顶掉一截、量尺可视区跟着变矮，而且每换一条高度还会跳。搬成浮卡之后它**不占布局高度**，选与不选之间画布尺寸恒定。分工也因此清楚了：带子是常驻的列举，浮卡是「看这一条」的临时窗口。「点任意一条…」那句提示留在带子里（它说的是带子的用法）。
- **浮卡的 left 跟着左侧量尺走**：`renderRulers` 现写一个 `--crop-ruler-left` 到 stage 上，CSS 用 `calc(var(--crop-ruler-left) + var(--space-3))`。量尺宽度是变量（切出档两层 92、其余一层 46），按最宽那档在 CSS 里写死内衬会让浮卡永远偏右一截。右下角是缩放/粒度那条工具带，所以浮卡落左下角，两者不打架。
- **两处接线**：浮卡挂在画布上而画布三档都在，所以离开通信档要自己收起（`setMode` 与 `renderCommDetail` 各一道）；`pointerdown` 的浮层白名单从 `.crop-tools` 扩成 `.crop-tools, .crop-comm__detail`，在浮卡上按下不再起拖、抬起也不再 `pick(null)` 清掉当前选择。
- 浮卡本体比带子里的常驻块多一层毛玻璃与投影（与 `.crop-tools` 同一套 `--glass-surface-bg` / `--surface-backdrop-filter`）：它盖在格子上面，没有这两层就分不出「浮在图上的窗口」和「图里的一块」。

## 2026-09-08 — 平面视图：另两档默认收起详情栏、粒度开关改名并成组

- **切到「通信查询 / 负载热力」时默认收起右侧详情栏**（`setMode`）。它答的是「你选中的那个对象是什么」—— 刚切过来还没选任何东西，那一栏就是一句「未选中」的空话，却占着 336px；而这两档恰恰最吃画布宽度（一条通信要横跨几个 stage 块，热力要看整片色分布）。⚠️ 不写 `rightPinnedClosed`：那面旗子记的是**用户**的表态，这里是页面替他收的 —— 所以随后在画布上点中任何东西，`cro:select` 那条会照常把它推回来，正是「点了才出现」。回到「配置寻优」档时，若此刻确有选中对象就还原。
- 顶栏那两枚面板开关的按下态原先只由它们自己的 click 维护，`setMode` 与 `cro:select` 收放右栏时会与实际状态脱节。把同步器提成 `syncPanelButtons`，三处共用一份。
- **画布右下角的粒度开关改名**：自动 / 卡 / 整机 → **自适应 / Rank 视角 / 整机视角**。「卡 / 整机」两个字单看像是在选一个*对象*（哪张卡、哪台机器），而这一组换的是**整幅图按什么单位画**；「自适应」也比「自动」说清了它自动的是什么。置灰理由与悬浮说明一并跟着改口径。
- **让这三档一眼读成一组**：原先靠一条右分隔线把它与缩放那几枚隔开 —— 分隔线只说得出「这边和那边不是一回事」，说不出「这三枚是一起的」，而且它压在 `.segmented-control` 自带的那圈描边上，把右边那条框线换成了另一种颜色。现在去掉分隔线，把那圈框线加重到 `--border-default`、底色压深一档（工具条本身是半透明玻璃底，muted 那档几乎与它同色，框自然看不出来）。

## 2026-09-08 — 配置关系平面：模型下拉之下新增「配置」预设选择

- `js/config-relation-plane.js` — 格内专家编号的阈值从「每卡 ≤ 8 个」放宽到 32：判据改成折行数上限 `EXPERT_ROWS_MAX = 8`（`EXPERT_CHIP_MAX = EXPERT_COLS_MAX × EXPERT_ROWS_MAX`），因为宽度早已封顶、真正的代价是格子高度。EP=8 那类配置（256 ÷ 8 = 每卡 32 个）从此仍逐个铺得出 E0–E255，代价是行高 22px → 139px、纵向世界长几倍；横向不变，缩放时照旧由 `TEXT_MIN_K` / `NODE_BUDGET` 退回整格上色。

- `config-relation-plane.html` / `js/config-relation-observer.js` / `js/config-relation-plane.js` — 左栏模型选择下方增加一枚「配置」下拉：新增 `CONFIG_PRESETS` 表（模型预设持结构常量，配置预设持「这一次怎么跑」的并行/批次数），首档是各模型的默认配置，openPangu 2.0 下增加「cinnnnnndy 配置」（EP 切出档 · DP16 × PP4 × TP2 × CP1 = 128 卡 · EP=8 → EDP 2）。整份预设经 `importConfig` 一次落地再配平，不逐个 `set`；手拨任一控件后下拉自落到「自定义」，不让预设名停在一份已被改过的配置上。

## 2026-09-08 — 通信查询档：改成压在画布上方的一条带子，链路三档染整条

- **通信档不再收画布。** 原先切到通信档整块换掉画布，两条量尺跟着一起没了。改成与热力档同构 —— 三档共用同一幅平面，通信那一段只是压在它上方的一条带子，画布与量尺照常显示。理由是后续的播放要在集群矩阵上用线画通信的方向与范围：那张矩阵必须一直在，带子只负责列举与选中，指到平面上的活由画布干。`setMode` 里那次「等一帧再夹边界、再重铺」因此对三档一律做（换档只改带子高度，画布可视区跟着变）。
- **三档链路（机内 HCCS / 机内+机间 / 机间 RDMA）从染 2px 竖线改成染整条底色**，并在 bar 右端补一组三色图例。原先只染范围读数左边那条细线，扫一行根本看不出来 —— 而「一个阶段里哪几条要出机器」正是这一档最该一眼看出的东西。三档底色都混在 `--surface-2` 上而不是吃满饱和度：hover 与选中还要靠明度往上提两档，底色吃满就没余量了；选中态另外把文字压到全对比度，橙底上单靠亮度认不出来。
- **事件条改成两行、整条 32px、两行都是 11px。** 第一行只写事件名（`PP Recv`），第二行写「模块 · 范围」（`Stage 入口` / `PP × 2`）。一行平铺时四段文字排成一长条，一个阶段七八条要横滚两屏；折成两行后每条窄一半，一屏基本装得下，扫读时眼睛只过第一行。顺带撤掉了序号胶囊 —— 箭头方向加左右次序已经说清顺序。
- **阶段名（前向 / 反向 / 梯度同步）从独立标题行挪到每一行左侧，11px，去掉描述文字。** 三个阶段各占一行标题白吃三行高度，而这一档现在要和画布分一屏；「进 stage → Attention → …」那句更是把下面那排事件条又用文字复述了一遍。详情卡同步改成横排 wrap（长的两条独占一行、短的四条并排），五条竖堆要一百多像素。

## 2026-09-08 — 平面视图：顶部量尺的高度改由 js 单点驱动

- **修顶部量尺第二行（MoE / Dense）被裁掉**。上一轮把 `RULER_TOP_H` 从 46 加到 52 再到 60，但量尺盒子的高度写死在 css 的 `.crop-ruler--top { height: 46px }` 里、没人同步 —— 盒子还是 46 且带 `overflow: hidden`，刻度本身虽然算出了 38px 的高度，第二行照样被裁在盒外。所以「加高了却还是看不见」。
- 改成由 `renderRulers` 每帧写内联（`rulerTop.style.height = RULER_TOP_H`），css 里那条降级为脚本没跑起来时的兜底（值同步到 60）。两个数各写各的迟早会再错开一次，单点驱动才不会重犯。

## 2026-09-08 — 平面视图：整机名收进行标两行、列名居中加高、EP 口径进表单格

- **撤掉整机行顶上那条「整机 N · rank a–b」标签带**（`.crop-nodecap` 及 `NODE_CAP_RATIO` / `NODE_CAP_MIN_PX`）。它与左边那个「整机 N」行标说的是同一件事，同屏出现两次纯属重复；而且它横贯整个 stage 块、每 8 行来一条，把「一整套专家」那层颜色压在下面。rank 区间改成**行标的第二行**，与整机号右端对齐叠在一起（`.crop-rowlabel__sub`，0.82em、透明度 0.72，跟着 js 写的父级字号走）。格子因此也不再让出顶上那一条，整行都归格子。
- **`rowMetrics` 加 `two` 分支**：整机行的行标是两行，每行只分到 0.44 个行高（原先单行是 0.95）；两行按 ≥10px 放不下就退回一行、只写整机号 —— 两行都挤成蚂蚁不如少说一句。行标道的宽度也跟着分两档量（`labelTextW` / `labelTextWNode`）：卡粒度只按 `rank 2047` 留，整机粒度按更长的 `rank 2040–2047` 留，不再用一个全局最大值让卡粒度凭空多留一截。
- **顶部量尺第二层加高并改成两行居中**：`RULER_TOP_H` 52 → 60（第二层 30 → 38，两行各要一整个行高再加行距）；`.crop-tick--col` 的 `align-items` 从 `flex-start` 改成 `center`。左对齐时「Layer12」与「MoE」右端参差不齐，一排刻度扫过去像是没对准；居中之后每一列读起来是一枚完整的标牌。左缘那道竖线仍是刻度线本身，标的还是这一列的左缘。
- **「EP 口径」那一格改成幂等重挂**（`mountEpModeField`，在 IIFE 与每次 `cro:change` 各跑一次）。原先只在 boot 之前插一次，而 `controller.mount()` 在 boot 里 `innerHTML = ""` 重建 `#croMoeSteppers`，那一份会被冲掉 —— 于是三档按钮仍留在 MoE 区标题右侧。现在它是 `#croMoeSteppers` 里的第一格、`grid-column: 1 / -1` 独占一行，与其余 stepper 落在同一套两列轨道上。按钮本体自始至终是同一个节点，主脚本挂在 `#croEpMode` 上的 click 委托与 onChange 同步原样有效。

## 2026-09-08 — 平面视图：「负载热力」从占位改成真的热力图

- **热力档不再换图，只换格子的颜色**：横轴仍是层、纵轴仍是卡，rank × layer 交叉的那一格按选中的度量从冷蓝烧到火红（`HEAT_RAMP` 六段插值，亮度单调上升、中途绕开绿色 —— 绿在这套色板里是「安全」的语义色）。收画布另起一幅新图的话，读者要重新找一遍位置，颜色再准也读不出所以然。
- **五枚度量胶囊接上开关**（内存占用 / 计算量 / 计算时间 / 空闲时间 / 通信时间），配一条现写的色阶图例：两端是**这幅平面自己的**最轻与最重，换度量、换配置都重算。
- **数值由 `buildHeat` 按当前配置与集群位置现算**，不是实测：结构那一半（参数量、激活、FLOPs、通信量）由切法直接推得；位置那一半是模型 —— 专家路由不均（确定性哈希，同一份配置每次画出来一样）、机内 HCCS 与跨机 RDMA 差一个数量级、每台机器自己的链路与降频抖动。这句话写进面板脚注常驻在图旁边，不只留在注释里。
- **降级档**：缩到画不出格子时（`drawCells` 为假），热力按行成带铺色（`HEAT_LOD_BUDGET`）—— 配置寻优档退成空白还剩行列高亮带可读，热力档退成空白就等于在「先缩到看全局」这一步上什么都不给。
- 热力档下专家胶囊与「一套专家」着色一起让位（两套编码叠在同一格上谁也读不清），「按完整一套专家区分着色」那枚开关整枚收起。

## 2026-09-08 — 平面视图：行标道按需留宽、顶部列名拆两行

- **stage 块之间那条「行标道」改成按这一帧的行标字号现算**（`rowMetrics` / `laneWorldAt` / `syncGeometry`），不再是一个拍脑袋的世界下界（原先 `ROW_LABEL_W 106 + STAGE_GAP 48 = 154`，k=1 时就是 154 屏幕像素，而写下「整机 1024」只要 68）。现在道宽 = 最长那串行标的实测宽度按当前字号等比换算 + 一点余量：屏幕上恒定 60 出头，**恰好够写、不多留**。
- **行标画不出来的那一档，道缩回 `GAP_MIN_PX`（14 屏幕像素）**：`showRowText` 为假时（行矮到写不下字）留一大片空地却不写字，是这条道最没有道理的一种状态，此时只留一条把两个块分开的最小缝。
- **余量改成跟着字号缩放**（`LANE_SLACK` 以探针字号为单位）：写成固定世界像素的话，缩到 10% 时余量在屏幕上只剩两像素，而 css 那 8px 右内衬同样缩掉了，字就贴着块的左缘甚至被截。
- **行标的字号、显隐、道宽三者收进同一份度量 `rowMetrics(k)`**：render 画字、`laneWorldAt` 留道、`zoomBy` 算不动点三边各判一次，迟早会出现「留了道却不写字」或「写了字却没道」的错位。
- **删掉上一轮那个 √2 量化**，改成在 `zoomBy` 里**正确补偿不动点**（`lanesBefore`）。量化本是为了压住「道宽连续变化 → 每滚一格画面横向漂一点」，代价是道宽最多虚胖 41%，与「不多留」直接冲突。根因其实是 `view.x = px - (px - view.x)·r` 把行标道也当成了会缩放的内容 —— 而它的屏幕宽是恒定的。现在先剥掉光标前面那几条道，只让**块内偏移**参与缩放，再按缩放后的道宽加回去；漂移从根上没有了，道宽也就可以取精确值。`fit()` 相应改成按真实占宽收敛（误差每步收到 ~0.35 倍，四步足够）。
- **顶部量尺的列名拆成两行**：层号一行、FFN 类型一行（`.crop-tick__name` / `.crop-tick__ffn`）。列宽只有几十像素，"Layer45 · Dense" 挤成一行时被截掉的恰好是后半截那个 Dense/MoE —— 而那是这一列最该先读到的一件事。顶部量尺第二层因此从 24 加高到 30（`RULER_TOP_H` 46 → 52）。
- **列宽下界跟着改成按较长的那一行量**（`nameW`）：原先按整串 "Layer45 · Dense" 量，凭空多出快一倍的宽度，整幅平面也就横着拉长一倍。

## 2026-09-08 — 平面视图：中栏加「配置寻优 / 通信查询 / 负载热力」三档

- **中栏从「一张画布」变成「一个壳 + 三档内容」**（`.crop-center` + `.crop-modes` 页签，见 `js/config-relation-plane.js` 的中栏外壳段与 `css/config-relation-plane.css` 末尾新增段）。三档做成中栏自己的页签而不是顶栏的第四、第五档：顶栏那三枚（关系视图 / YAML / 文档）换的是整块板面，而这三档换的只有中栏 —— 左边那份配置表单、右边那份选中详情，三档读的是同一个东西。yaml / 文档两档改成连壳一起收，否则板面让给 yaml 之后中栏顶上还浮着一条页签。切档回画布**不调 `fit()`**，只夹一次边界再重铺：切一趟页签就抹掉用户的缩放与位置，等于每次去看一眼通信都要重新找回刚才在看的那几张卡。
- **「通信查询」档：把并行配置翻译成一个 step 的跨卡通信序列。** 事件清单按 `Profiling_Insight_and_Tool/GPT问答总结-config待办.md` 的「层内前向与反向通信」「层外或参数同步通信」两张表列，分前向 / 反向 / 梯度同步三行，前两行的事件条做成箭头（前向朝右、反向掉头朝左），梯度同步不给方向 —— 它是 step 末尾的一次，不在层内的时间轴上。逐步播放按钮先置灰（播放效果未接），事件条本身可点，点开看单条详情。
- **每条事件的组大小、rank 步长、跨机与否全部由 `topology.counts` 现算，不查表。** rank 编址是 `stage×ranksPerStage + edp×ranksPerDp + ep×ranksPerEp + inner`，所以每个域在 rank 轴上都是等差数列；拿跨度和 `ranksPerNode` 一比就分出机内 HCCS / 机内+机间混合 / 机间 RDMA 三档 —— 这正是「同一个 TP=2，摆得对与摆得不对差一个数量级」的全部原因。混合那一档单列，因为它最容易被配置调出来、也最容易漏看。TP / CP 两个域的步长按 EP 口径分档给（mf 档下 TP 被 EP 那一维吃掉，见主脚本 `shardOf`）。「谁和谁」以当前选中的那张卡为锚，没选中时取 0 号卡。
- **「负载热力」档先只摆度量口径**（内存占用 / 计算量 / 计算时间 / 空闲时间 / 通信时间），按钮置灰、悬浮说明写清各自接上之后拿什么数据填：前两个可由当前配置直接推算，后三个要一次真实 profiling。

## 2026-09-08 — 平面视图：静息态右栏改成「当前配置评估」（L-A~L-D 四层）

- **没选东西时右栏不再只写一句「点点看」，而是评估手上这份配置能不能跑。** 选中态答的是「这个对象是什么」，静息态该答的是「这份配置怎么样」—— 后者才是打开这一页的第一个动机，也是左栏拨完十几个数之后每次都要重新问一遍的事。标题从「未选中」换成「当前配置评估」，原来那句操作提示挪到评估之后当页脚（它仍是这幅平面唯一的操作说明，不能丢）。
- **四层判据分开列、不揉成一个总分**，每行右侧带证据标（公式 / 估算 / 未建模）：
  - **L-A 可行性** —— 配得通（并行维乘积 = Total Rank，注明 EP 口径与 VPP 都不进乘积）、切得开（逐条列出头数 ÷ TP、Ulysses 的 TP×CP、Ring 的 2×CP、两个 intermediate、层数分段、专家 ÷ EP、Top-K 上界）、跑得下。
  - **L-B 容量** —— 最紧的卡、**stage 间不平衡度**（最满 ÷ 最空）、**压不掉的那部分**（底座 + 四项预留的分解）、距 70% 波动线的余量。
  - **L-C 效率** —— 流水线气泡 `(PP−1)/(micro_batch_num×VPP)`（闭式解）、重计算算力开销（经验系数，「按层数」档按重算层占比线性折算）、一步过的 token、以及**吞吐 / step time 如实写「未建模」**。
  - **L-D 风险与可信度** —— TP×CP 域是否跨机（当硬红线用，不是线性扣分）、EP all-to-all 域横跨几台、逐条列出 `topology.warnings`、以及这份估算本身的证据等级（卡型 HBM 是占位值时降到 D）。
- **总判定条**是整栏唯一铺底色的元素：一句话给出「预计可跑 / 余量偏紧 / 已越预警线 / 预计 OOM」，等级完全由容量定 —— 前两关到得了这里就已经是过了的（不自洽的配置根本发不出 `cro:change`）。下面各行只染读数不铺底，十几行一起铺会让「偏满」看着比「越界」还吵。
- **`config-relation-capacity.js` 新增只读导出 `global.croCapacityModel`**（`measureAll` / `THRESHOLD` / `LEVEL_LABEL` / `levelOf`）。评估栏不自己算显存，直接吃容量栏那份 `measure` —— 六段、1F1B 在飞份数、四项预留、LoRA / 分片 / 精度三处分支只应该有一份实现，「同一页两处各报一个不同的显存数」是这一页最不能出的错。纯只读：不碰 `pinnedStage`、不触发 `render`。
- `onChange` 里补一句 `if (!relation) renderDetail()`：静息态评的是**当前配置**，配置变了必须跟着重算；选中态由 `cro:select` 驱动，与配置无关，不必重跑。
- 指标行的分隔线用相邻兄弟选择器而不是 `:first-of-type` —— 小节的第一个子元素是标题 `div`，`:first-of-type` 会落在它身上，第一行指标反而留着一条多余的顶线。

## 2026-09-07 — 平面视图：dense 格的斜纹换成纯色

- **`.crop-cell[data-ffn="dense"]` 从 45° 斜纹（`repeating-linear-gradient`）改成一个纯色底**（比基色淡一档：前景 4% 对 9%）。一屏几千个格子、每个都要合成一遍渐变，是这幅图上最贵的一笔纯装饰开销；纯色绘制几乎不要钱。语义一点没丢 —— dense 格与 MoE 格的区别本来就是「里面什么都没有」（MoE 格要么摆着专家编号、要么整格上着那一套的颜色），用底色的浓淡说比用纹理说更直接。
- 规则位置随之从文件末尾挪到 `.crop-cell:hover` **之前**：它现在写的是 `background` 而不是 `background-image`，两条选择器权重相同（0,2,0），排在 hover 后面会把悬浮反馈整个盖掉。

## 2026-09-07 — 平面视图：缩放性能（重绘节流 + 节点预算 + 气泡按需拼）

缩放几下就卡的根因不是图形复杂，是**每个 rAF 都把可见区几千个 DOM 节点全量重建一遍**。滚轮一圈发几十个事件就排了几十次全量重铺，每次几十毫秒，于是越滚越卡。四处一起改（都在 `js/config-relation-plane.js`）：

- **交互期的重绘改成 leading + trailing 节流**（`scheduleRenderSoft`，170ms 最小间隔 / 110ms 停手补一帧）。缩放、拖动、拉窗口、收放左右栏这四条连续手势全部改走它；配置变了、选择变了仍走 `scheduleRender()`，那两件事必须当帧见效。手还在动的时候画面靠 CSS transform 自己缩放 —— 内容还在，只是边缘可能暂时空一截、行标字号暂时不是终值，手一停就补准。
- **加一条节点总数预算 `NODE_BUDGET`（9000）**，管的是「格子 + 格内专家胶囊」。原先只按格子数算（`CELL_BUDGET`），而一个格子可以带 8 枚胶囊 —— 2000 格就是 18000 个节点。超了整幅退掉胶囊、改成整格上色（与 `chipsOK` 同一个降级方向），格子本身照画。`CELL_BUDGET` 一并从 16000 收到 12000。
- **格子的悬浮气泡改成悬浮时现拼**（`cellTip` + `stage` 上一条 `pointerover`）。那串文案有五六行、要拼四五个模板字符串，原先每个格子都预拼一遍 —— 一帧几千个长字符串，而其中至多有一个会被人看到。时序上安全：主脚本的气泡层委托在 `document` 的冒泡阶段，这条挂在 `stage` 上必定先跑。
- **削掉每个节点的属性与样式写次数**：`place()` 从四次逐属性赋值改成一次 `cssText`（字号一并交给它写）；专家胶囊从 6 个 `data-*` 减到 2 个（`kind` / `expert`），`rank` / `stage` / `layer` / `epRank` 挂到**父格子**上、`pick()` 顺着 `closest` 取 —— 一格四枚胶囊时这一条就省 16 次属性写；跨 EP 的整机行不再每行多查一次 `expertsOfEpRank`。
- **`.crop-cell` 加 `contain: layout style paint`**：一屏几千格，containment 把每格的排版与绘制圈在自己盒子里。三项都安全 —— 格子绝对定位、尺寸由内联样式写死，内容本来就 `overflow:hidden`，里面没有计数器。

顺带修一处几何抖动：**行标道宽度量化到 √2 的整数次幂**（`laneAt`）。它随缩放变宽，而缩放的「以光标为不动点」是按旧几何算的，连续变化时每滚一格画面都要横向漂一点点。量化之后几何只在几个离散档位上跳一次，档内完全不动。「适配」相应从分段直解改成四步收敛（量化后档位有限，两三步就到位）。

## 2026-09-07 — 平面视图：滚动条收细、名字不弹气泡、行标道有屏幕下限

- **左右两栏的滚动条改成细杆**（`scrollbar-width: thin` + 6px 无轨 webkit 条，静息 14% / 悬浮 30% 的前景色）。浏览器默认那条带轨道的灰条在 344px 的栏里是一块**版式元素** —— 它自带一条竖直边界，读起来像给表单又划了一列。样式照抄集群矩阵那套，同一页面里滚动条只该有一种长相。
- **PP Stage 占位条与左侧 EDP 带同一副长相**：去掉圆角、补一道左分隔线。两者是同一件事的横竖两版（「这一段是谁」），不该一个圆角一个直角。
- **「整机 24 · rank 192–199」那条标签带不再铺底色**，透出画布底色，高亮也只动字。它横贯整个 stage 块，一上底就是一条实心横条、每 8 行来一条，满屏读起来像条纹表格，把「一整套专家」那层颜色压了下去；行的边界由整机之间那道 `NODE_GAP` 的缝交代，不必再靠色带。
- **行标 / 整机标签带 / 顶部 layer 刻度不再挂 `data-tip`**：它们就是一个名字，气泡里那几行（PP / DP / EP / 专家区间）在右栏写得更全，而沿着一列行标或一排刻度扫过去时每划过一格就弹一次，是这幅图上最吵的一处。格子（`.crop-cell`）的气泡留着 —— 那里问的是「这张卡 × 这一层」，右栏答不了。
- **顶部量尺的 layer 高亮不再把刻度线染蓝**：那道竖线标的是「这一列的左缘在这儿」，跟着选择变蓝会被读成「这条界线也被选中了」；相邻两列同时命中时，两根蓝线还会把中间那一列夹成一个假的选中块。高亮只动字与底色。
- **删掉「E12–E27」区间胶囊**（`.crop-expert--range`）。每卡专家太多、或整机行跨了几个 EP 时原先铺一枚区间胶囊，但区间既不是可点的对象、也答不出「这一片属于哪一套」，读的人还得把两个编号在脑子里减一遍。这一档现在整格交给颜色（`data-paint="bg"`），要编号就点进去看右栏；列宽也不必再为它留余量。
- **stage 块之间那条「行标道」有了屏幕下限**（`layout.laneAt(k)` / `syncX(k)`）。行标的字号随缩放**反向**放大（屏幕上恒 ≥10px），而这条道是世界单位、跟着缩放一起缩 —— 缩到 30% 时道只剩 46 屏幕像素，10px 的「整机 1024」写不下，就压到左边那个 stage 块上去了。现在道宽取 `max(世界下界 154, laneMinPx / k)`，`laneMinPx` 按最长的那串行标实测（`rank N` / `整机 N`）：无论怎么缩放，屏幕上都写得下一整个行标。块的 x 因此依赖 k，`clampView` / `render` / `fit` 三处各同步一次。「适配」也跟着改成分段直解而不是一次除法 —— 世界宽度自己也是 k 的函数（`A(k) = n·max(laneBase·k, laneMinPx) + k·sumW`），一次除法解出来的 k 装不下。

## 2026-09-07 — 平面视图：左右栏不再互压、专家标签折行、纵向分区缝

- **左右两栏的每一节钉成 `flex: 0 0 auto`，栏体自己纵向滚**（`css/config-relation-plane.css`）。`.cro-region` / `.cro-section` 自带 `min-height: 0`（原版要它们在 grid 里能收），搬进这两条**纵向 flex** 的栏之后同一条属性变成了「可以被压到比内容还矮」—— 一压，表单行、单卡容量框、结构条就各自溢出自己的盒子，读起来正是那些交叉重叠。
- **补回 `.cro-advanced-panel[hidden] { display: none }`**：平面视图那条把面板改成两列网格的规则是三个类（0,3,0），压得过 `config-relation-observer.css` 里 `[hidden]` 那条（0,2,0）—— 折叠因此根本收不起来，收起态的那几格与下一区的标题直接叠在一起。这是左栏最显眼的一处重叠。
- **「高级选项」从按钮改成一句蓝字**：独占一整行，去掉 `.btn` 的底色、边框与内衬（那块比字大一圈的热区），上方的占位 label 一并撤掉；悬浮气泡（`data-hint`）由 `js/config-relation-plane.js` 在 boot 之后摘掉 —— 气泡说的是「这个折叠里收了哪几格」，展开一看就知道，却要靠那块热区才触发得到。展开态的箭头留着：它是这一行唯一能读出「开着还是收着」的东西。
- **左栏改配置时不再自动掀开折叠**（`js/config-relation-observer.js` 的 `highlightLinkedChanges`）。344px 的窄栏里拨一格数字就弹开一整片折叠，版面在手底下跳，比「3 秒高亮播在看不见的地方」更难受；折叠是用户收起来的，页面不该拿一次联动去推翻它。判据挂在 `board.is-plane` 上，原版 `config-relation-observer.html` 那边一如既往地掀。
- **PP Stage 占位条画回普通矩形**：右端那个 12px 的切角箭头吃掉标签宽度、把靠边的 stage 名截掉，而块与块的先后横轴本身就给得出来。
- **格内专家标签一行最多 4 枚，多了折行，行高自适应增高**（`EXPERT_COLS_MAX`）。原先 8 枚铺一行的格子有列名的两倍宽，整幅平面横着拉长一倍，扫一行卡要拖两屏；改成宽度封顶、高度往下长之后，横轴回到「一列 ≈ 一个层名」的尺度 —— 纵向本来就要滚，行是这幅图里更便宜的那根轴。折行位置由实测的「一整行胶囊宽」限死（`--crop-chips-w` / `--crop-chip-w` 写在 `.crop-world` 上），列名更长时多出来的宽度全让给内衬，不会被第 5 枚挤上来。行高 `CELL_H` 因此从常量变成 `layout.cellH`，量尺与渲染一并改读它。
- **纵向补出两级分区缝**：EDP 副本之间 `DP_GAP` 12 → 36（3 倍），副本内整机与整机之间新增 `NODE_GAP` 12（原先没有缝，整机档一片连续行读不出机器边界）。两者拉开 3 倍差，缩小之后先看到副本、再看到机器。缝一律落在整机边界**之间**、整机行内部一条不插，所以整机行的高度仍恰好等于它那几行卡之和，切粒度时几何不动、视口不跳这条守住了；除不尽的配置（整机档本就不成立）不插整机缝，免得画一个假边界。
- **PP stage 块横向间距 24 → 48**：块与块之间还站着下一块的行标（"整机 1024" 那一串），缝太窄时两者贴在一起，缩小之后「整机 xx」读不完整。
- **「专家着色」更名为「按完整一套专家区分着色」**：四个字读起来像「给每个专家上色」，而着色的单位其实是**一套**（一个 MoE 层 × 一个副本）—— 名字里带上单位，开关就不必靠悬浮提示才说得清。关掉时胶囊退到中性灰蓝 `--crop-set-neutral`，不再退到 `--pto-model-deck-moe`（#EA580C 橙红）：那是「MoE」的语义色，满屏橙红读起来像一片告警，而这一档的本意恰恰是「不表达任何分组」。

## 2026-09-07 — 配置关系观测：切 EP 口径锁住卡数、超参考规模给软提示

- **切 EP 口径时锚在 Total Rank 上倒推，而不是拿换算完的 DP 反推卡数**（`js/config-relation-observer.js` 的 `setEpMode`）。三档说的本来就是同一批硬件（架构参考 §6.1 那 2048 卡），只是读法不同 —— 切出/mf 档读作 DP512，正交档读作 DP8，一张卡都不该多也不该少。原先靠 `convertDpAcrossEpMode` 的换算式隐式维持这一条：默认档上结果没错（实测 876 次真实切档，卡数变化 0 次），但那个换算会撞 DP 的量程上限、除不尽时还要 floor 一下，任何一次夹取都会把卡数悄悄带偏，而页面上没有任何东西会喊出来。现在把切档前的 world 记下来当锚，交给 `fitParallelWorld` 倒推；倒推只动 dp / tp / cp / pp（正交档下 EP 进乘积，也算 cluster 维），`totalLayer` / `routedExpert` / `topK` / `sharedExpert` 一概不碰 —— 换个读法不该改动模型结构与 MoE 分组（同样 876 次切档，结构字段改动 0 次）。够不到目标时（mf→切出档下 EP 被 DP 的整除约束逼降那一类）把 `totalRank` 照实写回：`reconcile` 的 totalRank 分支只把 world 拉向目标、不回写那个字段，不补这一下 Total Rank 那格会显示一个几何上并不存在的数。
- **卡数涨过参考配置时出一条软警告**（`warn()`）。往下调是常事（拿小规模试算、或就是要看 512 卡怎么摆），所以只在**大于**参考值时说；往上调却几乎总是手滑 —— 加减键一按就是 ×2，DP 512 → 1024 那一下屏幕上只动了一个数字，卡数已经翻倍，而容量柱、YAML 的 `worker_num`、集群矩阵的格子数全都跟着按新规模在报。判据取预设自己的 `defaults.totalRank`（openPangu 2048 / Qwen2-7B 8），不写死 2048，换模型时自己跟着换。软警告不拦截：大于仍然允许，只是要说一声。
- **平面视图左侧量尺在正交档下补回内层**（`js/config-relation-plane.js`）。内层在不在，判据从「EDP 与 DP 是否重名」改成 **EP > 1**：正交档 EDP ≡ DP = 8，按原判据整层塌掉 —— 512 行只挂 8 个 DP 带子，中间那 64 行 EP 一个标都没有，而这一档恰恰是拿来跟切出档对读的，一对读纵轴就瞎了。内层写什么两档不同，因为那几行的**身份**不同：切出 / mf 档一行是表单里那个 DP 的一个成员（DP = EDP × EP），写 DP 号；正交档 EP 是独立的一根轴、副本内没有「内层 DP」这回事，写 EP 号。角上跟着从 `EDP | DP` 变成 `DP | EP`。稠密模型（EP=1）仍然只有一层，不造一个自己等于自己的层级。

## 2026-09-07 — 平面视图：量尺对齐修正、行标最小 10px、专家着色开关

- **修正两条量尺与内容错位**：刻度是量尺盒子的**子元素**，它的 `left / top` 以量尺左上角为原点，而算出来的是画布坐标 —— 顶部量尺整体右移了左尺宽、左侧量尺整体下移了顶尺高，写样式时没减掉，刻度因此与内容整整错开一个量尺的宽 / 高。两处各减一次偏移量即可。
- **删掉左上角常驻说明块**（`.crop-legend`）：口径改由量尺、悬浮提示与右栏承担，画布上不再压一块常驻文字。
- **行标（rank / 整机号）在屏幕上恒 ≥ 10px**：字号写成世界单位的 `10/k` —— 缩小多少就把字放大多少，屏幕尺寸因此不变；上界是这一行自己的高度（0.95 行高），装不下就不画，而不是画一行认不出的蚂蚁。行标框宽度随之从 62 放到 106 世界像素（k=0.5 时字号涨到 20 世界像素，"rank 2047" 那一串要将近 90），stage 块间距从 48 收到 24 找回这部分宽度。
- **顶部刻度写全名**：不再有 `L0` 这种简写，一律 `Layer 0 · Dense` / `Layer 4 · MoE` —— 刻度是用来对位置的，简写差掉的正是「这一层是 Dense 还是 MoE」这条最该先读到的信息。
- **auto 粒度改由「专家标签写不写得下」决定**：写不下的那一刻（缩放低于 50%）卡粒度已经没有信息可给了（一行只剩一个色块），直接聚成整机 —— 行高 8 倍，标签、行标、整机标签带全都重新写得下。回到卡粒度要多缩放 15%（迟滞），免得停在阈值上抖。
- **整机行顶上加一条「整机 N · rank a–b」标签带**：整机行有 8 倍高，腾得出这一条；它答的是「这一整行是哪台机器、含哪几张卡」，比只在左边挂一个行标更直接，也让「一行 = 一台机器」有个可见的边界。行本身的 y 与高度一点没动（切粒度不跳视口这条不能破），变的只是行内部的分配。行标文案一并改成中文「整机 N」。
- **右下角新增「专家着色」开关（默认开）**，并改变上色落点：着色开着时颜色只落在**标签**上（编号与颜色本来就是同一件事的两半，分开摆等于让底色去和选中高亮抢面积）；只有标签画不出来 —— 缩得太小、或整机档一行代表 8 张卡 —— 才退回给格子底上色，那时颜色是唯一还能表达「这一片属于哪一套专家」的东西。

## 2026-09-07 — 平面视图：着色改按「一套完整专家」、双层动态量尺、列名精确化

- **着色单位从 EP 组改成「一套完整专家」**（`js/config-relation-plane.js` + `css/config-relation-plane.css`）：同色的一整块 = **一个 MoE 层 × 一个 DP/EDP 副本**，块里那 EP 张卡各持一片、合起来正好是 routedExpert 个专家的一整套。相邻两套换色，而相邻有两个方向（左右换层、上下换副本），所以索引取 `(层号 + 2×副本号) % 4`，两个方向的邻居必定不同色。EP 不再单独着色 —— EP 是这一套**内部**的分片，同一套里每张卡同色、编号不同，那才是「一整套被切开」该有的读法。行标那道 EP 分段色条随之撤掉，不留两套互相打架的颜色。
- **列名精确化**：`layer0` 改成 `Layer0 · Dense` / `Layer4 · MoE`；并把 **Emb / Norm / Head** 三个端点结构补成真正的列（首段前一列、末段后两列）。它们不是层，但确实驻留在首 / 末那一段 PP 的卡上，不画就等于那两段凭空少一块。点它们发的是 `kind:"segment"` + `wholeColumn`，与在「典型 Layer」里点整列同一条通路，主脚本按 stageAnchor 反查驻留的卡，联动照旧。列宽因此除了专家标签，还要能写下最长的列名，两者取大。
- **双层动态量尺**：顶部 `PP | Layer`、左侧 `EDP | DP`（正交档 / 无 MoE 时退回一层 `DP`），交汇的角上写纵轴口径。量尺按**屏幕坐标**铺、不进 transform：字号始终一样大、始终贴边，缩放时变的只有刻度疏密 —— 一格挤不下就每 2 / 5 / 10 / 20 格标一次（`niceStep`，1-2-5 梯子）。原先画在世界里的 stage 条与列标随之移除：它们一缩小就跟着糊掉，而量尺的职责恰恰是在看不清内容时也答得出「这是哪一段、第几个副本」。切出档下左侧内层还给出这一段对应的 DP 号（`DP = EDP × EP` 那个换算，表单里那个 DP 512 与矩阵行数 8 对不上，是本页最容易读错的一处）。
- **平移加边界**：可以拖到内容大半出屏，但不允许拖到整片空白 —— 内容与可视区至少留 120px 重叠（`clampView`）。此前拖过头就只剩狂点「适配」一条路。
- **左栏表单改两列网格**：原版那几行是 flex-wrap，在 344px 里会按剩余宽度随机折行、标签互相挤，展开「高级」时更是叠成一片。改成固定两列：每格宽度确定、折行位置不随窗口跳，「高级」面板 `grid-column: 1 / -1` 独占整行。Cluster 那两个挂载点仍是 `display:contents`，七格控件因此落在同一套轨道上。
- **EP 口径三档收进 MoE 表单**：原先游离在 MoE 区标题右侧，搬进左栏后既不像标题也不像表单。补一个「EP 口径」label 收成整行的一格，摆在那几枚 stepper 之前（它是「下面这些数字怎么读」的前提）。只做 `#croMoeSteppers` 的兄弟节点 —— `controller.mount()` 每次都会 `innerHTML = ""` 重建那个容器。
- **右栏「计算节点」只留相关列 + 端点列补层号范围**：有选择时隐掉不相关的列（336px 里摆五列，每列都窄到读不出算子名）；`Emb` / `Norm` / `Head` 的名字补成 `Emb（PP0 · L0~L11）`，答出「它到底在哪一段」。

## 2026-09-07 — 平面视图：整机（node）粒度 LOD、EP 着色、列宽按专家标签自适应

- **整机粒度（`js/config-relation-plane.js`）**：画布多一档 LOD —— 一行 = 一台整机（`ranksPerNode`，硬件事实 8 卡；它是这幅图里唯一的物理单位，机内走 HCCS、出机走 RDMA，「机器边界切断了哪根通信轴」只有在这一档上才画得出来）。整机行在世界坐标里**恰好等于它那 8 行卡的高度**，所以切粒度时几何一动不动、视口不跳。右下角新增「自动 / 卡 / 整机」开关：自动档按屏幕行高带迟滞切换（低于 8px 聚成整机、高于 11px 散回卡），也可手动锁死。只有 `ranksPerDp % ranksPerNode === 0` 时才给这一档（否则一台机器会骑在两个 EDP 副本之间，聚出来的格子会说谎），不成立时按钮置灰并在悬浮里说明原因。整机行的高亮直接读主脚本 `resolveRelation` 已有的 `rel.nodes`；点击仍发首卡的 `kind:"rank"`（共用口径层没有 node 这一档，不为一页去改它），右栏按整机口径写标题与读数（本机卡范围 / EP 覆盖 / 合持专家区间）。
- **列宽按专家标签实测（同上）**：列宽不再写死 30px，改由隐形探针量出 `E{routedExpert-1}` 胶囊的真实宽度反推 —— 每卡专家数 ≤ 8 时逐个铺满，更多时铺成区间胶囊（`E12–E27`），标签不再被折成「+N」。
- **EP 着色（同上 + `css/config-relation-plane.css`）**：8 色循环，色值与 `training-run-twin.css` 的 `.ep-tint-N` 同源。格子底色与专家胶囊都从同一个 `--crop-ep` 变量取，因此「8 种色」只有 8 行 CSS；行标右缘一道色条，单组是实色、整机跨组时按覆盖顺序切成分段渐变 —— 那几段本身就是「机器边界把 all-to-all 域切成了几块」的答案。跨组时格子底色留空，不染成其中一个组的颜色。
- **画布去掉点阵底**：这幅图本身就是一张密网格，底下再垫一层点会读成两套网格互相干扰；「无限」由 pan/zoom 表达，不靠背景纹理暗示。
- **legend 常驻一句专家编号口径**：专家编号**每层各自**从 0 数到 routedExpert-1，一个 EDP 副本里的 EP 张卡各持一份、合起来才凑齐一整套；换层、换副本都从 0 重来。顺着一个副本往下扫时编号从 0 走到 255，看着像在累计，其实那是 EP 分片 —— 这是这幅图最容易被读错的一处，格子的悬浮提示里也写了同一句。
- **降级改成整幅一起降**：格子预算从「边铺边扣」改成「先数总量再决定铺不铺」，避免出现「前两个 stage 有格子、后两个空着」这种读起来像后半段没有卡的画面。缩放下界放宽到 0.03，大配置的「适配」才真的装得下。

## 2026-09-07 — 关系观测器新增「平面视图」版式（config-relation-plane.html）

- **新增 `Profiling_Insight_and_Tool/training-run-twin-standalone/config-relation-plane.html`**：与 `config-relation-observer.html` 共用同一套 CSS/JS，html 只差三处（title、多一份 css、多一个 script）。原版四域并置 + 关系连线在配置一大时摆不下、也读不出交叉关系，这一版按 `pic/新视图.png` 改成三栏——左：配置（表单 / 代码 / 整网图 三档）；中：无限画布，stage × layer × rank × 专家穿插在同一平面，交叉关系由「同一格」表达，连线整层收掉；右：选中对象（rank / layer / stage / 专家）的详情，单卡容量与典型 Layer 整块搬进这里。
- **新增 `js/config-relation-plane.js`**：只做重排（appendChild，节点 / id / 已挂监听器全部保留）与画布。点击一律走主脚本导出的 `window.croSelect(payload)`，与点集群矩阵格子同一个入口、同一份 payload；重绘只吃 `cro:change` / `cro:select`。画布按视口裁剪虚拟化，46 层 × 上千行也只铺可见的那几百格。集群矩阵 / Layer 刻度带 / MoE 宫格没有删，移进离屏的 `.crop-engine`（`applyRelation` 仍往里铺高亮，MoE 绑定态也存在那儿）。
- **新增 `css/config-relation-plane.css`**：规则一律写成 `.cro-board.is-plane …` 压过原版的 `.cro-board …`，`config-relation-observer.{css,js}` 与原版 html 一个字都没改；脚本没加载成功时自动退回原版四域排布。
- **`launch.html`**：「配置与关系观测」卡改成 split 卡，「四域」/「平面」两个入口。

## 2026-09-07 — 关系观测器卡型号选项改名与 950PR 容量

- **`js/config-relation-observer.js` CARD_SPECS**：第一档 `910b-32` 的显示名由「910B」改为「910」（label 昇腾 910），只改名、32 GB 容量不变；第三档 `950` 改为「950PR」，单卡容量由占位的 64 GB 改为 **128 GB**（取 KNOWLEDGE.md §3.1 的整片 DDR 128 GB）。容量栏、yaml 的 `max_device_memory`、口径浮层都读 `card.hbmGB`，随之按 128 重算。
