...
在 DeepSeek 和 PyPTO 里，"循环"至少有四层，不能混着看：
...
- 控制流图就是源码 `for/if` 的直接可视化
- 泳道图上的一个 task 就等于源码里的一个循环体
...
> **关键问题不是盯着 `for` 语法，而是先问：这一层循环到底在重复什么模板。**
...
> 当 PTO 里看到同类算子一再出现时，先不要急着理解为"代码里又写了一层循环"，很可能只是模型架构本来就在重复调用同一类 kernel。
...
**模型层 loop 决定"为什么同类 kernel 会不断被调用"，源码层 loop 决定"一个 kernel 怎样分批处理数据"，tile loop 决定"每一批数据怎样在核内被逐块算完"，而 controlflow 则是在描述这些循环最终保留下来的执行骨架。**
