...
Compute Graph Viewer（内部代号 **PTO**）是面向 Ascend NPU 计算图的本地可视化工具，纯静态前端（HTML + CSS + Vanilla JS），加载编译器输出的 JSON 进行 DAG 交互分析。入口为 `launch.html`，对应三条功能路标：

- **精度调试**：`index.html` — Pass IR 计算图，逐阶段追踪编译优化
- **架构洞察**：`mvp/index.html` — 大模型整网架构，L1→L2→L3 折叠展开
- **性能调优**：`visual-test.html` — 算子源码计算流可视化（beta）
...
| 状态 | 特性 | 备注 |
|------|------|------|
| ✅ | 启动页 / demo 导航入口（三卡片） | `launch.html` |
| ✅ | 文件夹 handoff（IndexedDB + FileSystemDirectoryHandle） | launch → index |
| ✅ | 主站深色主题 Design Token | `css/style.css` |
| ✅ | MVP 暗色主题（v0.5 修复） | `mvp/styles.css` 已与主站对齐 |
...
| 状态 | 特性 | 解决什么问题 |
|------|------|------------|
| ✅ | 算子源码可视化（`visual-test.html`，beta） | 从图节点跳转到源码行 |
...
```
pto/
├── launch.html                        # 启动页 / demo 导航（三卡片）
├── index.html                         # 精度调试：Pass IR 计算图
├── visual-test.html                   # 性能调优：算子源码计算流（beta）
├── CHANGELOG.md                       # 开发日志
├── css/
...
│   ├── antv-flow.js                   # X6 封装（visual-test.html 专用）
│   └── test-syntax.js                 # 实验脚本（未接入主应用）
├── mvp/
...
| `mvp/app.js` | X6 封装：L1→L4 折叠展开，V3/V3.2 切换 | `buildScene()`, `buildExpandableGroup()` |

**脚本加载顺序**（`index.html`）：`colormap` → `parser` → `layout` → `renderer` → `nav_index_builder` → `app` → `nav` → `controlflow-data` → `controlflow`
...
| `mvp/x6.min.js` | MVP 独占 | — |
| `mvp/data.js` | MVP 独占 | — |
| `js/antv-flow.js` | visual-test.html 专用 | — |
