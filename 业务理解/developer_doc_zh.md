...
前面术语里已经说过，`token_x` 不是原始文本，也不是 token id，而是当前 kernel 真正收到的一批数值向量。很多第一次接触这个链路的人，会在这里卡住：用户明明输入的是一句中文，为什么到了算子代码里会变成一个 `[t, h]` 的张量？如果只用术语解释，很容易还是觉得抽象，所以这里直接给出一张流程图。

```mermaid
flowchart LR
    A["用户输入一句话<br/>今天天气不错"]
    B["Tokenizer<br/>把文本切成 token"]
    C["token id<br/>把 token 变成整数编号"]
    D["Embedding<br/>把编号查表变成向量"]
    E["前面网络层继续处理<br/>不断更新 hidden state"]
    F["整理形状<br/>为当前 kernel 的输入做准备"]
    G["token_x<br/>当前 kernel 真正收到的输入张量"]

    A --> B --> C --> D --> E --> F --> G
```

这张图想表达的核心只有一句话：`token_x` 是"模型内部的数字语言"，不是"用户写下来的那句话本身"。用户输入的文本先会被 tokenizer 切成 token，再把 token 变成离散的整数编号。整数编号仍然不能直接拿来做矩阵乘法，所以模型会通过 embedding table 把每个编号查成一条浮点向量。到了这一步，文本已经从"可读的字符"变成了"模型能计算的数值表示"。

但这还不是最终的 `token_x`。embedding 只是第一版向量，后面还会经过前面若干层网络继续处理，例如归一化、线性变换、注意力、残差相加等。每经过一层，这个 token 对应的向量都会更新一次，于是就形成了当前阶段的 hidden state。因此，`token_x` 更准确的定义是：某些 token 在"到达当前 kernel 时"的隐藏状态矩阵。

如果把这个过程再说得更直白一点，可以写成下面这条链：
...
-> token id
-> embedding 向量
-> 前面网络加工后的 hidden state
-> token_x
```

因此，当 graph 工具里出现 `token_x` 节点时，最合适的阅读方式不是把它想成"一句话"或者"一个词"，而是把它理解成"当前 kernel 的输入激活矩阵"。矩阵里的每一行，都代表某个 token 在当前阶段的一条 hidden state 向量。
...
flowchart TD
    ...
    D["按请求和序列组织<br/>shape = [b, s1, h]"]
    E["把前两维压平<br/>t = b * s1"]
    F["供当前 kernel 使用<br/>shape = [t, h]"]

    A --> B --> C --> D --> E --> F
```

这里可以把三个维度理解成下面的意思。`b` 是 batch size，也就是一次同时处理多少条请求；`s1` 是当前阶段参与计算的 token 数；`h` 是 hidden size，也就是每个 token 对应的向量长度。于是 `[b, s1, h]` 的含义就是："一批请求里，每个 token 位置都有一条长度为 `h` 的 hidden state 向量。"

很多 kernel 在实现时，会把前两维合并，写成：
...
这样做不是因为语义变了，而是因为对很多算子来说，更关心的是"当前总共有多少个 token 位置要算"，而不是这些 token 来自第几条请求。比如原本 shape 是 `[2, 4, 7168]`，表示 2 条请求、每条 4 个 token 位置、每个位置 7168 维；压平之后就会变成 `[8, 7168]`。这时第一维的 8，不再强调 batch 边界，而是强调"当前共有 8 行 hidden state 要参与这个 kernel 的计算"。

这也是为什么文档里会反复强调：图里看到的一行，不是一个汉字本身，也不是一个 token id，而是"某个 token 在当前网络阶段对应的一条 hidden state 向量"。从产品设计角度看，把这层含义解释清楚非常重要。因为只有先明白 `token_x` 真正在表示什么，后面开发者看到 matmul、reshape、cache 写回这些图节点时，才会知道它们处理的是哪一类数据，以及 shape 的变化到底在说明什么。
