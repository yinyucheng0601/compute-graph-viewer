(function attachOpFusionApp() {
  ...
  function setFusionPanelOpen(open) {
    if (!els.fusionPanel) return;
    els.fusionPanel.hidden = !open;
    if (els.fusionToggle) {
      els.fusionToggle.classList.toggle('is-selected', open);
      els.fusionToggle.setAttribute('aria-pressed', open ? 'true' : 'false');
    }
  }

  const FUSION_PANEL_MIN = 300;
  const FUSION_PANEL_MAX = 760;
  const FUSION_PANEL_WIDTH_KEY = 'op-fusion-panel-width-v1';

  function setFusionPanelWidth(width) {
    if (!els.fusionPanel) return;
    const row = els.mainRow;
    // 预留给中间可视化区与已展开的其它面板的最小空间
    const rowMax = row ? row.clientWidth - 360 : FUSION_PANEL_MAX;
    const upper = Math.max(FUSION_PANEL_MIN, Math.min(FUSION_PANEL_MAX, rowMax));
    const w = Math.round(Math.max(FUSION_PANEL_MIN, Math.min(width, upper)));
    els.fusionPanel.style.setProperty('--op-side-panel-w', `${w}px`);
    return w;
  }

  function initFusionPanelResize() {
    const handle = els.fusionResizer;
    const panel = els.fusionPanel;
    if (!handle || !panel) return;

    // 恢复上次保存的宽度
    try {
      const saved = Number(localStorage.getItem(FUSION_PANEL_WIDTH_KEY));
      if (Number.isFinite(saved) && saved > 0) setFusionPanelWidth(saved);
    } catch (_) { /* ignore */ }
    ...
    const onMove = (event) => {
      if (!dragging) return;
      // 面板在右侧：向左拖动（dx<0）加宽，向右拖动收窄
      setFusionPanelWidth(startW - (event.clientX - startX));
    };
    ...
    const onUp = (event) => {
      ...
      document.body.classList.remove('op-resizing-panel');
      handle.releasePointerCapture?.(event.pointerId);
      window.removeEventListener('pointermove', onMove);
      window.removeEventListener('pointerup', onUp);
      try {
        const w = parseInt(panel.style.getPropertyValue('--op-side-panel-w'), 10);
        if (Number.isFinite(w)) localStorage.setItem(FUSION_PANEL_WIDTH_KEY, String(w));
      } catch (_) { /* ignore */ }
      fit();
    };
    ...
    handle.addEventListener('pointerdown', (event) => {
      ...
      document.body.classList.add('op-resizing-panel');
  ...
  function renderOperatorDetailPanel(node) {
    const detailContent = els.detailBody || document.getElementById('op-detail-body');
    if (!detailContent) return;
    if (els.detailMeta) els.detailMeta.textContent = node.typeLabel || node.label || '算子详情';
  ...
  function initInteractions() {
    ...
    // 融合算子推荐面板：由右上角 icon 控制开合，与算子详情面板相互独立
    els.fusionToggle?.addEventListener('click', () => {
      setFusionPanelOpen(els.fusionPanel?.hidden !== false);
    });
    els.fusionClose?.addEventListener('click', () => setFusionPanelOpen(false));
    // 融合算子推荐面板：左侧手柄拖动调整宽度
    initFusionPanelResize();
    // 算子详情面板：关闭按钮
    els.detailClose?.addEventListener('click', hideDetailPanel);
    // 面板初始状态：融合推荐默认展开，算子详情待点击后出现
    setFusionPanelOpen(els.fusionPanel?.hidden !== true);
    hideDetailPanel();
  ...
  function init() {
    Object.assign(els, {
      ...
      graphPane: document.querySelector('.op-graph-pane'),
      detailPanel: document.getElementById('op-detail-panel'),
      detailBody: document.getElementById('op-detail-body'),
      detailMeta: document.getElementById('op-detail-meta'),
      detailClose: document.getElementById('detail-close'),
      fusionPanel: document.getElementById('op-fusion-panel'),
      fusionToggle: document.getElementById('fusion-toggle'),
      fusionClose: document.getElementById('fusion-close'),
      fusionResizer: document.getElementById('fusion-resizer'),
      mainRow: document.querySelector('.op-main-row'),
    });
