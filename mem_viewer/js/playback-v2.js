...
import { initMemoryArchitectureV2, renderMemoryArchitectureV2 } from './memory-architecture-v2.js';
...
function syncColorPanelCollapsed(collapsed) {
  if (!colorPanel || !colorPanelToggle) return;
  colorPanel.classList.toggle('is-collapsed', collapsed);
  colorPanelToggle.setAttribute('aria-expanded', collapsed ? 'false' : 'true');
}
...
function goToStep(step) {
  ...
  renderMemoryArchitectureV2(step);
...
colorPanelToggle?.addEventListener('click', () => {
  syncColorPanelCollapsed(!colorPanel?.classList.contains('is-collapsed'));
});
...
/* ============================================================
   Initialize
   ============================================================ */
initMemoryArchitectureV2();
initPanelResizer();
syncFloatingToolbarState();
syncColorPanelCollapsed(true);
goToStep(0);
loadGraph().then(() => {
