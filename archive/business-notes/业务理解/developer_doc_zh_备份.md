...
`AST` 是抽象语法树。可以把它理解成“Python 代码的语法结构图”。它关心的是代码是怎么写的，比如这里是一个函数定义，那里是一个 `for` 循环，这里是一个函数调用，而不是关心最终要不要发起一次 `matmul`。

`IR` 是中间表示。可以把它理解成“编译器内部真正拿来处理和优化的图”。它比源码更接近执行，也比最终机器代码更容易分析。官方 README 里也强调了 PyPTO 是通过多层 IR 逐步向下编译的。

“惰性执行”或者 “Lazy Execution” 指的不是偷懒执行，而是“先不编译，等第一次真正调用时再编译”。官方前端开发文档和 `entry.py` 都明确说明了这一点：`JitCallableWrapper` 会把真正的 parse 和 compile 延迟到第一次 `__call__`。
...
`X` 是数学公式里的记号，表示隐藏状态矩阵。到了代码里，它通常会写成 `token_x` 或 `x_in`。这两个名字不是新的算法对象，只是数学公式 `X` 的代码变量名。DeepSeek README 已经明确写了：MLA Prolog 的输入是 hidden state `X`，而代码里的 `token_x` 是这个输入张量的实现名。

`隐藏状态` 可以理解成“大模型在当前阶段对一个 token 的内部数值理解”。它不是汉字、单词，也不是 token id，而是一串浮点数。模型每往后算一层，这串数都会被更新一次，所以叫“状态”。
如果非常粗暴地类比：
  大模型收到用户输入的原始文本
  -> tokenizer 变成 token
  -> embedding 变成第一版向量
  -> 经过前面若干层网络处理
  -> 得到当前这一层看到的 hidden state

所以代码里的 `X` 是隐藏状态矩阵，本质上是在说：这不是原始文本数据，而是“模型当前拿在手里的特征向量集合”。
  `上层网络` 就是“在这个算子之前，模型已经算过的那些部分”。
  对 mla_prolog_quant_compute(...) 来说， `X`是它的输入。那说明在它之前，已经有别的模块把文本一路处理成了 [t,h] 的张量，然后把这个结果交给它。
  


`tensor`是什么？ 

开发阶段 → tensor 是"计算图节点" 开发者写的是声明式描述。有四个属性：dtype、shape、format（内存排布）、name。

运行阶段 → tensor 是"内存里真实的数值块" 第一次调用时触发编译，走完整链路： Tensor Graph → Tile Graph → Block Graph → Execute Graph → NPU 可执行代码 最终 MPMD 方式调度到 NPU 各处理器核。tensor 变成真实内存地址上的数组，框架在 Tile Graph 阶段自动把大 tensor 切分成能放进 L1/UB 缓存的小块执行。

内存里的`tensor`实际数值从哪来？ 
数值来源分四类：
1、输入激活（如 token_x）：前面层的计算输出，原始文本 → tokenizer → embedding → 若干层网络 → 当前算子
2、权重（如 w_dq, w_uk）：checkpoint 文件，运行前加载到设备内
3、缓存（如 kv_cache）：前面时间步写入的状态
4、辅助配置（如 cos/sin, cache_index）：按位置规则预生成，或运行时根据当前 token 位置算出
...
如果把上面六步放回产品设计里，可以得到一个非常明确的结论：这不是“一张图加几个开关”能解决的需求，而是一个新的分析入口。

我建议把这个入口理解成 `Frontend Explorer`。它不替代现有 Pass 图，而是补齐 Pass 图前面的链路。三类入口的职责可以这样划分：

`index.html` 负责展示编译之后的 Pass IR，回答“编译器优化以后图长什么样”。

`mvp` 负责展示模型结构，回答“整网架构从语义上如何组织”。

新的前端六阶段入口则负责回答“源码是如何一步步变成初始图的”。

从交互上看，这个入口最合理的布局应该是三栏：

- 左栏是源码面板，负责回答“这段节点对应哪几行代码”。
- 中栏是图视图，负责展示当前阶段的结构。
- 右栏是 Inspector，负责展示节点属性、变量生命周期、动态维度绑定、语义标签等元信息。

顶部再提供一个 stage switcher，让用户能在 `Source`、`Python AST`、`Doc AST`、`Liveness`、`Initial IR`、`Lazy Execution` 之间切换。

这样设计的好处是：用户的注意力始终围绕同一个真实案例，不是在六张互不相关的页面之间来回跳转。

## 7. 第一版最值得落地的能力

如果以产品节奏来安排，我不建议第一版就把六个阶段全部做到位。更现实的路线，是先交付最能证明价值的三件事情。

第一，能让用户在同一个 DeepSeek kernel 上切换 `Source`、`Python AST` 和 `Initial IR`。只要这三层打通，用户就能第一次真正看见“Python 代码不是直接变成 Pass 图”的全过程。

第二，要把 `loop_unroll`、`semantic_label` 和动态维度 `t` 清楚展示出来。因为这三类信息最能体现 PyPTO 前端的特色，也最能解释为什么一个简单的源码浏览器不够用。

第三，`Initial IR` 在竖排模式下采用接近 TensorBoard 的 tensor 展示方式。也就是把普通 tensor 压成 edge data，而不是继续画成一排排独立节点。这样在复杂案例中，可读性会明显更高。

如果这三件事能先跑通，后续再把 `Doc AST`、`Liveness`、`Lazy Execution` 补齐，会是自然扩展，而不是返工。
